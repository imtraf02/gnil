...
void Application::scheduleNotificationShellRefresh() {
  ...
  DeferredCall::callLater([this]() {
    ...
    m_bar.refresh();
    if (m_panelManager.isOpenPanel("control-center") || m_panelManager.isOpenPanel("sidebar")) {
...
void Application::reconcileOutputSurfaces() {
  ...
  // stacking (for example screen corners above the rail) is identical in both cases.
  ...
  if (m_systemBus != nullptr) {
    ...
    try {
      ...
      m_networkService->setChangeCallback(
          [this, shouldRefreshControlCenter, refreshNexusServiceUi](
              const NetworkState& state, NetworkChangeOrigin origin
          ) {
    ...
    try {
      ...
      auto refreshBluetoothUi = [this, shouldRefreshControlCenter, refreshNexusServiceUi]() {
        m_bar.refresh();
        if (shouldRefreshControlCenter()) {
          m_panelManager.refresh();
        }
    ...
    if (m_bluetoothService != nullptr) {
      try {
        ...
        m_bluetoothAgent->setRequestCallback(
            [this, shouldRefreshControlCenter, refreshNexusServiceUi](const BluetoothPairingRequest& /*request*/) {
              if (shouldRefreshControlCenter()) {
                m_panelManager.refresh();
              }
              refreshNexusServiceUi();
...
void Application::initSessionBusServices() {
  ...
  if (m_bus != nullptr) {
    ...
    m_trayService->setMenuToggleCallback([this](
                                             const std::string& itemId, float /*contentScale*/, float anchorSurfaceX,
                                             float anchorSurfaceY, float /*anchorWidth*/, float /*anchorHeight*/
                                         ) {
      m_panelManager.togglePanel(
          "tray-menu",
          PanelOpenRequest{
              .anchorX = anchorSurfaceX,
              .anchorY = anchorSurfaceY,
              .hasExplicitAnchor = anchorSurfaceX >= 0.0f && anchorSurfaceY >= 0.0f,
              .hasAnchorPosition = anchorSurfaceX >= 0.0f && anchorSurfaceY >= 0.0f,
              .context = itemId,
          }
      );
    });
