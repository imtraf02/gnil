...
void TrayService::requestMenuToggle(
    const std::string& itemId, float contentScale, float anchorSurfaceX, float anchorSurfaceY, float anchorWidth,
    float anchorHeight
) const {
  if (m_menuToggleCallback) {
    m_menuToggleCallback(itemId, contentScale, anchorSurfaceX, anchorSurfaceY, anchorWidth, anchorHeight);
  }
}
