...
public:
  ...
  using MenuToggleCallback = std::function<void(const std::string&, float, float, float, float, float)>;
  ...
  void requestMenuToggle(
      const std::string& itemId, float contentScale = 1.0f, float anchorSurfaceX = -1.0f, float anchorSurfaceY = -1.0f,
      float anchorWidth = -1.0f, float anchorHeight = -1.0f
  ) const;
