...
private:
  ...
  void layoutAppIcons(Item& item, float width, float height);
  [[nodiscard]] std::vector<std::string>
  workspaceWindowGlyphs(const Workspace& workspace, std::size_t displayIndex) const;
  ...
  std::size_t m_maxWindowIcons = 5;
  bool m_wasFocusedOutput = true;
  bool m_activeUsesFocusedColor = true;
  Node* m_container = nullptr;
  std::vector<Workspace> m_cachedState;
  std::vector<std::string> m_cachedDisplayKeys;
  std::vector<WorkspaceWindowAssignment> m_cachedWindowAssignments;
  ...
  Box* m_activeIndicator = nullptr;
  float m_activeIndicatorFromX = 0.0f;
  float m_activeIndicatorFromY = 0.0f;
  float m_activeIndicatorFromWidth = 0.0f;
  float m_activeIndicatorFromHeight = 0.0f;
