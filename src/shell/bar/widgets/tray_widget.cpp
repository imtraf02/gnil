...
namespace {
  ...
  TrayAnchorGeometry trayAnchorGeometry(const InputArea& area) {
    float left = 0.0f;
    float top = 0.0f;
    float right = 0.0f;
    float bottom = 0.0f;
    Node::transformedBounds(&area, left, top, right, bottom);
    const float width = std::max(1.0f, right - left);
    const float height = std::max(1.0f, bottom - top);
    return TrayAnchorGeometry{
...
void TrayWidget::rebuild(Renderer& renderer) {
  ...
  for (const auto& item : m_items) {
    ...
    area->setOnClick([this, itemId, areaPtr](const InputArea::PointerData& data) {
      ...
      if (data.button == BTN_LEFT) {
        // Caelestia treats a tray menu as a rail popout on primary click.
        // Items without a DBus menu retain their normal activation action.
        if (m_tray->itemUsesDBusMenu(itemId)) {
          const auto anchor = trayAnchorGeometry(*areaPtr);
          requestPanelToggle("tray-menu", itemId, anchor.centerX, anchor.centerY);
        } else {
