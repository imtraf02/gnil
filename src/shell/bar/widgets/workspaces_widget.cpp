...
WorkspacesWidget::WorkspacesWidget(CompositorPlatform& platform, wl_output* output, Options options)
    : m_platform(platform), m_output(output), m_displayMode(options.displayMode),
      ...
      m_focusedOutputOnly(options.focusedOutputOnly), m_showWindows(options.showWindows),
      m_maxWindowIcons(std::max<std::size_t>(1, options.maxWindowIcons)), m_focusedColor(options.focusedColor),
...
bool WorkspacesWidget::shouldShowWorkspaceLabel(const Workspace& workspace, std::string_view label) const noexcept {
  if (effectiveDisplayMode() == DisplayMode::None || effectiveDisplayMode() == DisplayMode::State || label.empty()) {
    return false;
  }
...
void WorkspacesWidget::applyItemVisualStyle(Item& item) {
  ...
  for (Glyph* glyph : item.appIcons) {
    if (glyph != nullptr) {
      glyph->setColor(workspaceTextColor(item.visualWorkspace));
    }
  }
}
...
void WorkspacesWidget::doUpdate(Renderer& renderer) {
  ...
  auto currentDisplayKeys = m_platform.workspaceDisplayKeys(m_output);
  auto currentAssignments = m_platform.workspaceWindowAssignments(m_output);
  ...
  if (!showWidget) {
    ...
    m_activeIndicator = nullptr;
    m_hoverProgress = 0.0f;
    if (!m_cachedState.empty() || !m_items.empty()) {
      cancelAnimation();
      m_cachedState.clear();
      m_cachedDisplayKeys.clear();
      m_cachedWindowAssignments.clear();
      m_items.clear();
  ...
  bool structuralChange = current.size() != m_cachedState.size()
      || currentDisplayKeys != m_cachedDisplayKeys
      || currentAssignments != m_cachedWindowAssignments;
  ...
  m_cachedDisplayKeys = std::move(currentDisplayKeys);
  m_cachedWindowAssignments = std::move(currentAssignments);
...
void WorkspacesWidget::rebuild(Renderer& renderer) {
    ...
    for (std::size_t i = 0; i < workspaces.size(); ++i) {
      ...
      entries.push_back(
          RebuildEntry{
              ...
              .exiting = false,
              .appIds = workspaceWindowGlyphs(workspace, i),
  ...
  for (std::size_t i = 0; i < entries.size(); ++i) {
    ...
    if (effectiveDisplayMode() == DisplayMode::State) {
      const float glyphSize = std::round(std::max(10.0f, baseSize * 0.68f));
      const std::size_t glyphCount = std::min(entry.appIds.size(), m_maxWindowIcons);
      const float glyphGap = std::round(2.0f * m_contentScale);
      const float glyphWidth = glyphCount == 0
          ? 0.0f
          : glyphSize * static_cast<float>(glyphCount) + glyphGap * static_cast<float>(glyphCount - 1);
      const float stateWidth = std::max(baseSize, glyphWidth + Style::spaceXs * m_contentScale * 2.0f);
      slot.inactiveWidth = stateWidth;
      slot.activeWidth = stateWidth;
      continue;
    }
  ...
  if (!m_minimal && effectiveDisplayMode() == DisplayMode::State) {
    m_activeIndicator = static_cast<Box*>(m_container->addChild(
        ui::box({
            .fill = m_focusedColor,
            .radius = workspacePillRadius(m_indicatorHeight, m_indicatorHeight),
            .width = m_indicatorHeight,
  ...
  for (std::size_t i = 0; i < entries.size(); ++i) {
    ...
    const float appIconSize = std::round(std::max(10.0f, pillHeight * 0.68f));
    const std::size_t iconCount = std::min(entry.appIds.size(), m_maxWindowIcons);
    item.appIcons.reserve(iconCount);
    for (std::size_t iconIndex = 0; iconIndex < iconCount; ++iconIndex) {
      auto glyph = ui::glyph({
          .glyph = entry.appIds[iconIndex],
          .glyphSize = appIconSize,
          .color = workspaceTextColor(ws),
      });
      glyph->measure(renderer);
      item.appIcons.push_back(glyph.get());
...
void WorkspacesWidget::recalculateItemMetrics(
  ...
  } else if (effectiveDisplayMode() == DisplayMode::State) {
    const float glyphSize = std::round(std::max(10.0f, baseSize * 0.68f));
    const std::size_t glyphCount = std::min(item.appIcons.size(), m_maxWindowIcons);
    const float glyphGap = std::round(2.0f * m_contentScale);
    const float glyphWidth = glyphCount == 0
        ? 0.0f
        : glyphSize * static_cast<float>(glyphCount) + glyphGap * static_cast<float>(glyphCount - 1);
    const float stateWidth = std::max(baseSize, glyphWidth + Style::spaceXs * m_contentScale * 2.0f);
    item.inactiveWidth = stateWidth;
    item.activeWidth = stateWidth;
  } else {
    const float minWidth = workspaceMainAxisMinWidth(baseSize, false);
    const float minActiveWidth = workspaceMainAxisMinWidth(baseSize, true);
    if (!item.showLabel) {
...
void WorkspacesWidget::startAnimation() {
  ...
  if (m_activeIndicator != nullptr) {
    m_activeIndicatorFromX = m_activeIndicator->x();
    m_activeIndicatorFromY = m_activeIndicator->y();
    m_activeIndicatorFromWidth = m_activeIndicator->width();
    m_activeIndicatorFromHeight = m_activeIndicator->height();
  }
  cancelAnimation();
  requestFrameTick();
  requestRedraw();
  m_animId = mgr->animate(
      ...
      [this](float t) {
        ...
        updateItemFlowPositions();
        applyItemLayouts();
        if (m_activeIndicator != nullptr && m_activeIndicator->visible()) {
          const float targetX = m_activeIndicator->x();
          const float targetY = m_activeIndicator->y();
          const float targetWidth = m_activeIndicator->width();
          const float targetHeight = m_activeIndicator->height();
          m_activeIndicator->setPosition(
              std::lerp(m_activeIndicatorFromX, targetX, t), std::lerp(m_activeIndicatorFromY, targetY, t)
          );
          m_activeIndicator->setFrameSize(
              std::lerp(m_activeIndicatorFromWidth, targetWidth, t),
              std::lerp(m_activeIndicatorFromHeight, targetHeight, t)
          );
        }
        updateContainerSize();
        if (root() != nullptr) {
...
void WorkspacesWidget::applyItemLayout(Item& it) {
  ...
  layoutAppIcons(it, itemW, itemH);

  if (m_activeIndicator != nullptr && it.workspace.active && visible) {
    m_activeIndicator->setVisible(true);
    m_activeIndicator->setFill(workspaceFillColor(it.visualWorkspace));
    m_activeIndicator->setPosition(it.area->x(), it.area->y());
    m_activeIndicator->setFrameSize(itemW, itemH);
    m_activeIndicator->setRadius(workspacePillRadius(itemW, itemH));
  }
  ...
  const bool showText = it.showLabel && it.appIcons.empty() && it.currentWidth + 0.5f >= it.inactiveWidth;
...
void WorkspacesWidget::layoutAppIcons(Item& item, float width, float height) {
  const std::size_t visibleCount = std::min(item.appIcons.size(), m_maxWindowIcons);
  if (visibleCount == 0) {
    return;
  }

  const float crossSize = m_isVertical ? width : height;
  const float mainSize = m_isVertical ? height : width;
  const float iconSize = std::round(std::max(8.0f, std::min(crossSize * 0.68f, 16.0f * m_contentScale)));
  const float gap = std::round(2.0f * m_contentScale);
  const float groupSize = iconSize * static_cast<float>(visibleCount) + gap * static_cast<float>(visibleCount - 1);
  const float mainStart = std::max(0.0f, (mainSize - groupSize) * 0.5f);
  const float crossStart = std::max(0.0f, (crossSize - iconSize) * 0.5f);

  for (std::size_t index = 0; index < item.appIcons.size(); ++index) {
    Glyph* icon = item.appIcons[index];
    const bool visible = index < visibleCount && groupSize <= mainSize + 0.5f;
    ...
    const float main = mainStart + static_cast<float>(index) * (iconSize + gap);
    icon->setPosition(m_isVertical ? crossStart : main, m_isVertical ? main : crossStart);
  }
}

std::vector<std::string>
WorkspacesWidget::workspaceWindowGlyphs(const Workspace& workspace, std::size_t displayIndex) const {
  if (!m_showWindows) {
    return {};
  }

  std::vector<std::string_view> keys;
  if (displayIndex < m_cachedDisplayKeys.size() && !m_cachedDisplayKeys[displayIndex].empty()) {
    keys.push_back(m_cachedDisplayKeys[displayIndex]);
  }
  if (!workspace.id.empty()) {
    keys.push_back(workspace.id);
  }
  if (!workspace.name.empty()) {
    keys.push_back(workspace.name);
  }

  std::vector<std::string> glyphs;
  glyphs.reserve(std::min(m_cachedWindowAssignments.size(), m_maxWindowIcons));
  for (const auto& assignment : m_cachedWindowAssignments) {
    if (!std::ranges::contains(keys, std::string_view(assignment.workspaceKey))) {
      continue;
    }
    const DesktopEntry entry = app_identity::resolveRunningDesktopEntry(assignment.appId, desktopEntries());
    glyphs.push_back(workspace_glyph::forApp(assignment.appId, entry.categoriesLower));
    if (glyphs.size() == m_maxWindowIcons) {
      break;
    }
  }

  if (glyphs.empty() && effectiveDisplayMode() == DisplayMode::State) {
    glyphs.push_back(workspace.occupied ? "app-window" : "circle");
  }
  return glyphs;
}
...
ColorSpec WorkspacesWidget::workspaceTextColor(const Workspace& workspace) const {
  if (effectiveDisplayMode() == DisplayMode::State) {
    switch (workspace_glyph::stateTone(workspace.active, workspace.urgent, workspace.occupied)) {
    case workspace_glyph::StateTone::Urgent:
      return colorSpecFromRole(ColorRole::Error);
    case workspace_glyph::StateTone::Active:
      return readableColorForFill(workspaceFillColor(workspace));
    case workspace_glyph::StateTone::Occupied:
      return m_occupiedColor;
    case workspace_glyph::StateTone::Empty: {
