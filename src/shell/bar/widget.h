...
public:
  ...
  // Morph every Material Symbol in this widget between outline and filled.
  // Panel-launching widgets use it to preserve the trigger across the drawer
  // transition without requiring per-widget icon plumbing.
  void setPanelActive(bool active, bool animate = true);
  [[nodiscard]] bool panelActive() const noexcept { return m_panelActive; }
