...
std::unique_ptr<Widget> WidgetFactory::create(
  ...
  // GNIL has one fixed Caelestia-style icon rail. Keep its visual language
  // icon-first regardless of legacy per-widget label settings.
  const bool iconRail = barPosition == "left";
  if (iconRail) {
    contentScale *= 1.25f;
  }
  ...
  if (type == "active_window") {
    ...
    auto widget = std::make_unique<ActiveWindowWidget>(
        ...
        iconRail ? ActiveWindowDisplayMode::IconOnly : parseActiveWindowDisplayMode(displayMode), showEmptyLabel
    );
  ...
  if (type == "battery") {
    ...
    const bool showLabel = !iconRail && (wc != nullptr ? wc->getBool("show_label", true) : true);
  ...
  if (type == "bluetooth") {
    const bool showLabel = !iconRail && (wc != nullptr ? wc->getBool("show_label", false) : false);
  ...
  if (type == "custom_button") {
    ...
    auto widget = std::make_unique<CustomButtonWidget>(CustomButtonWidget::Options{
        ...
        .label = iconRail ? std::string{} : trimSetting("label"),
  ...
  if (type == "sysmon") {
    ...
    SysmonWidgetOptions options{
        ...
        .showLabel = !iconRail && (wc != nullptr ? wc->getBool("show_label", true) : true),
  ...
  if (type == "taskbar") {
    TaskbarWidgetOptions options{
        ...
        .showWorkspaceLabel = !iconRail && (wc != nullptr ? wc->getBool("show_workspace_label", true) : true),
  ...
  if (type == "weather") {
    ...
    const bool showCondition = !iconRail && (wc != nullptr ? wc->getBool("show_condition", true) : true);
    const bool showTemperature = !iconRail && (wc != nullptr ? wc->getBool("show_temperature", true) : true);
  ...
  if (type == "workspaces") {
    const std::string defaultDisplay = iconRail ? "state" : "id";
    const std::string display = wc != nullptr ? wc->getString("display", defaultDisplay) : defaultDisplay;
    ...
    if (display == "id") {
      ...
      displayMode = WorkspacesWidget::DisplayMode::State;
    }
    ...
    WorkspacesWidget::Options options{
        ...
        .minimal = wc != nullptr ? wc->getBool("minimal", false) : false,
        .focusedOutputOnly = wc != nullptr ? wc->getBool("focused_output_only", false) : false,
        .showWindows = wc != nullptr ? wc->getBool("show_windows", true) : true,
        .maxWindowIcons = static_cast<std::size_t>(
            std::clamp<std::int64_t>(wc != nullptr ? wc->getInt("max_window_icons", 5) : 5, 1, 10)
        ),
    };
