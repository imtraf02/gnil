...
public:
  ...
  void setChromePanelState(wl_output* output, std::string_view barName, std::optional<ChromePanelState> state);
  // Notification chrome is published in output-local coordinates and is drawn
  // by the same full-screen SDF pass as the frame and panels.
  bool setChromeToastStates(wl_output* output, std::vector<ChromePanelState> states);
  void clearPanelActiveWidget();
...
private:
  ...
  // Chrome dimensions change independently of bar/widget configuration. Keep
  // that path in-place so changing frame thickness never recreates surfaces or
  // widget trees.
  void refreshChromeLayout();
  ...
  void setActivePanelWidget(BarInstance* instance, Widget* widget);
