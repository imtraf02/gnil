...
namespace {
  ...
  constexpr int kCompactCardHeight = 76;
  ...
  // Cards sit inside one shared chrome panel. The panel itself owns the frame
  // contact; these values are only the symmetric inset inside that material.
  constexpr float kPaddingX = Style::spaceSm;
  constexpr float kPaddingBottom = Style::spaceLg;
  ...
  constexpr std::string_view kGnilGlyphIconPrefix = "gnil-glyph:";
  ...
  constexpr int kMaxToastCardHeight = 360;

  [[nodiscard]] float notificationUiScale(const ConfigService* config) {
    if (config == nullptr) {
      return 1.0f;
    }
    return std::max(0.1f, config->config().accessibility.uiScale);
  }

  [[nodiscard]] float cardWidth(float scale) { return static_cast<float>(kCardWidth) * scale; }

  [[nodiscard]] float chromeEdgeInset(const ConfigService* config, std::string_view edge) {
    if (config == nullptr) {
      return 0.0f;
    }
    float inset = std::max(0.0f, config->config().shell.chrome.frameThickness);
    for (const auto& bar : config->config().bars) {
      if (bar.enabled && bar.position == edge) {
        inset = std::max(inset, static_cast<float>(std::max(0, bar.thickness)));
      }
    }
    return inset;
  }

  [[nodiscard]] float paddingTop(const ConfigService* config, float scale) {
    return chromeEdgeInset(config, "top") + Style::spaceSm * scale;
  }
  ...
  [[nodiscard]] std::uint32_t surfaceWidth(float scale, float innerPadX, float rightChromeInset) {
    return static_cast<std::uint32_t>(std::max(
        1, static_cast<int>(std::ceil(cardWidth(scale) + innerPadX * 2.0f + rightChromeInset))
    ));
  }
  ...
  [[nodiscard]] std::uint32_t fallbackSurfaceHeight(float scale) {
    const float totalHeight = maxToastCardHeight(scale) * kFallbackVisibleCards
        ...
        + paddingTop(nullptr, scale) + paddingBottom(scale);
  ...
  std::filesystem::path remoteIconCachePath(std::string_view url) {
    const std::filesystem::path cacheDir = std::filesystem::path("/tmp") / "gnil-notification-icons";
...
void NotificationToast::setChromeToastStateCallback(
    std::function<bool(wl_output*, std::vector<ChromePanelState>)> callback
) {
  m_chromeToastStateCallback = std::move(callback);
  for (auto& inst : m_instances) {
    if (inst != nullptr) {
      syncChromeHosting(*inst);
      inst->rebuildRequested = true;
      if (inst->surface != nullptr) inst->surface->requestLayout();
    }
  }
}
...
void NotificationToast::setSidebarOpen(bool open) {
      ...
      for (const auto& entry : m_entries) {
        ...
        const int32_t remainingMs = std::max<int32_t>(
            1, static_cast<int32_t>(std::ceil(
                   static_cast<float>(entry.displayDurationMs) * std::clamp(entry.remainingProgress, 0.0f, 1.0f)
               ))
        );
...
void NotificationToast::onNotificationEvent(const Notification& n, NotificationEvent event) {
  ...
  case NotificationEvent::Added:
    if (m_sidebarOpen || (m_notifications != nullptr && m_notifications->doNotDisturb())) {
      break;
    }
  ...
  case NotificationEvent::Updated: {
    ...
    for (std::size_t i = 0; i < m_entries.size(); ++i) {
      if (m_entries[i].notificationId == n.id && !m_entries[i].exiting) {
        ...
        m_entries[i].receivedTime = n.receivedTime;
        m_entries[i].receivedWallClock = n.receivedWallClock;
        refreshEntryGeometry(m_entries[i]);
        ...
        for (auto& inst : m_instances) {
          ...
          if (layoutChanged) {
            ...
            InputArea* rebuilt = buildCard(
                m_entries[i], &cs.cardContent, &cs.cardForeground, &cs.cardBackground, &cs.progressBar,
...
void NotificationToast::addPopup(const Notification& n) {
  ...
  entry.receivedTime = n.receivedTime;
  entry.receivedWallClock = n.receivedWallClock;
  ...
  entry.expanded = m_config != nullptr && m_config->config().notification.openExpanded;
...
void NotificationToast::addCardToInstance(Instance& inst, std::size_t entryIndex) {
  ...
  InputArea* card = buildCard(
      entry, &cs.cardContent, &cs.cardForeground, &cs.cardBackground, &cs.progressBar, &cs.actionsRowNode,
...
void NotificationToast::removeCardFromInstance(Instance& inst, std::size_t entryIndex) {
  ...
  if (cs.heightAnimId != 0) {
    inst.animations.cancel(cs.heightAnimId);
    cs.heightAnimId = 0;
  }
  if (cs.cardNode == nullptr) {
    return;
...
void NotificationToast::refreshEntryGeometry(PopupEntry& entry) const {
  ...
  if (!entry.expanded) {
    entry.toastBodyLines = StringUtils::isBlank(entry.body) ? 0 : 1;
    entry.height = static_cast<float>(kCompactCardHeight) * scale;
    return;
  }
  if (m_renderContext == nullptr) {
    entry.toastBodyLines = StringUtils::isBlank(entry.body) ? 0 : 500;
    entry.height = maxToastCardHeight(scale);
    return;
  }
...
float NotificationToast::layoutBottomForSurfaceHeight(float surfaceHeight) const {
  ...
  return std::max(paddingTop(m_config, scale), surfaceHeight - edgePadding);
}
...
float NotificationToast::cardSurfaceY(const Instance& inst, std::size_t entryIndex) const {
  ...
  if (items.empty()) {
    return bottom ? layoutBottom : paddingTop(m_config, scale);
  }
...
void NotificationToast::alignBottomStackToPlacementBottom() {
  ...
  const float scale = notificationUiScale(m_config);
  const float topPadding = paddingTop(m_config, scale);
...
void NotificationToast::collapseStack() {
  const float scale = notificationUiScale(m_config);
  const float layoutGap = kGap * scale;
  const float topPad = paddingTop(m_config, scale);
  const float placementBottom = maxPlacementBottom();
...
uint32_t NotificationToast::surfaceHeightForOutput(wl_output* output) const {
  if (m_wayland != nullptr && output != nullptr) {
    if (const auto* wlOutput = m_wayland->findOutputByWl(output); wlOutput != nullptr) {
      ...
      if (logicalHeight > 0) {
        const std::int32_t available = logicalHeight;
...
void NotificationToast::ensureSurfaces() {
  ...
  const auto selectedMonitors = notificationMonitors();
  const auto surfaceWidth = ::surfaceWidth(
      scale, horizontalInnerPad(scale), chromeEdgeInset(m_config, "right")
  );
  const std::uint32_t anchor = toastSurfaceAnchor(position);
  const ToastSurfaceMargins margins = toastSurfaceMargins(position, 0, 0, scale);
  ...
  std::erase_if(m_instances, [this, anyConfiguredPresent](const auto& inst) {
    ...
    const bool remove = anyConfiguredPresent && !shouldRenderOnOutput(*output);
    return remove;
  });
...
void NotificationToast::destroySurfaces() {
  for (auto& inst : m_instances) {
    if (inst != nullptr && m_chromeToastStateCallback) {
      (void)m_chromeToastStateCallback(inst->output, {});
    }
    if (inst->surface != nullptr) {
...
void NotificationToast::syncChromeHosting(Instance& inst) {
  const bool hosted = inst.chromeHosted
      || (m_chromeToastStateCallback && m_chromeToastStateCallback(inst.output, {}));
  if (inst.chromeHosted == hosted) return;
  inst.chromeHosted = hosted;
  inst.rebuildRequested = true;
}

void NotificationToast::publishChromeStates(Instance& inst) {
  if (!inst.chromeHosted || !m_chromeToastStateCallback || inst.surface == nullptr || m_wayland == nullptr) {
    return;
  }
  ...
  const float outputW = static_cast<float>(output->logicalWidth);
  const float outputH = static_cast<float>(output->logicalHeight);
  const float originX = position.find("left") != std::string::npos
      ? static_cast<float>(inst.surface->marginLeft())
      : outputW - static_cast<float>(inst.surface->marginRight()) - surfaceW;
  const float originY = position.find("bottom") != std::string::npos
      ? outputH - static_cast<float>(inst.surface->marginBottom()) - surfaceH
      : static_cast<float>(inst.surface->marginTop());
  const float scale = notificationUiScale(m_config);
  const float innerPad = Style::spaceSm * scale;
  float stackBottom = -1.0f;
  for (std::size_t i = 0; i < inst.cards.size(); ++i) {
    const auto& card = inst.cards[i];
    if (card.cardNode == nullptr || card.clipHeight <= 0.5f) {
      continue;
    }
    stackBottom = std::max(stackBottom, cardSurfaceY(inst, i) + card.clipHeight);
  }

  if (stackBottom <= 0.0f) {
    inst.toastPanelState.reset();
    inst.toastPanelTarget.reset();
    if (inst.toastPanelAnimId != 0) {
      inst.animations.cancel(inst.toastPanelAnimId);
      inst.toastPanelAnimId = 0;
    }
    (void)m_chromeToastStateCallback(inst.output, {});
    return;
  }

  const float topInset = chromeEdgeInset(m_config, "top");
  const float rightInset = chromeEdgeInset(m_config, "right");
  const float panelWidth = cardWidth(scale) + innerPad * 2.0f;
  const float localRight = surfaceW - rightInset;
  const float localTop = topInset;
  const float panelHeight = std::max(1.0f, stackBottom - localTop + innerPad);
  ChromePanelState desired{
      .rect = ChromeRect{
          .x = originX + localRight - panelWidth,
          .y = originY + localTop,
          .width = panelWidth,
          .height = panelHeight,
      },
      .radius = m_config != nullptr
          ? std::max(1.0f, m_config->config().shell.chrome.rounding)
          : Style::scaledRadiusXl(scale),
      // Top is the primary attachment axis. resolveJoinedShape also detects
      // the exact right-frame contact and selects the top-right corner mode.
      .edge = ChromeEdge::Top,
      .attached = true,
      .visible = true,
  };

  const auto submit = [this, &inst](const ChromePanelState& state) {
    if (!m_chromeToastStateCallback(inst.output, {state})) {
      inst.chromeHosted = false;
      inst.rebuildRequested = true;
    }
  };
  ...
  if (inst.toastPanelTarget.has_value() && inst.toastPanelTarget->rect == desired.rect
      && std::abs(inst.toastPanelTarget->radius - desired.radius) < 0.01f) {
    submit(*inst.toastPanelState);
    return;
  }
  ...
  const ChromePanelState from = *inst.toastPanelState;
  inst.toastPanelTarget = desired;
  Instance* const instPtr = &inst;
  inst.toastPanelAnimId = inst.animations.animate(
      0.0f, 1.0f, Style::animChromeSpatial, Easing::CaelestiaExpressiveSpatial,
      [this, instPtr, from, desired](float progress) {
        if (instPtr == nullptr || !m_chromeToastStateCallback) {
          return;
        }
        ChromePanelState sampled = desired;
        sampled.rect = ChromeRect{
            .x = std::lerp(from.rect.x, desired.rect.x, progress),
            .y = std::lerp(from.rect.y, desired.rect.y, progress),
            .width = std::max(1.0f, std::lerp(from.rect.width, desired.rect.width, progress)),
            .height = std::max(1.0f, std::lerp(from.rect.height, desired.rect.height, progress)),
        };
        sampled.radius = std::max(1.0f, std::lerp(from.radius, desired.radius, progress));
        instPtr->toastPanelState = sampled;
        if (!m_chromeToastStateCallback(instPtr->output, {sampled})) {
          instPtr->chromeHosted = false;
          instPtr->rebuildRequested = true;
        }
      },
      [instPtr, desired]() {
        instPtr->toastPanelAnimId = 0;
        instPtr->toastPanelState = desired;
      },
      inst.sceneRoot.get()
  );
  if (inst.surface != nullptr) {
  ...
  if (!inst.chromeHosted) {
    inst.chromeHosted = false;
    inst.rebuildRequested = true;
  }
}

void NotificationToast::updateInputRegion(Instance& inst) {
  if (inst.surface == nullptr) {
    return;
  }
...
void NotificationToast::setCardVisualHeight(Instance::CardState& card, float height) const {
  ...
  const float visibleHeight = std::max(1.0f, height);
  const float width = card.cardNode->width();
  card.clipHeight = visibleHeight;
  card.cardNode->setFrameSize(width, visibleHeight);
  if (card.cardContent != nullptr) {
    card.cardContent->setFrameSize(card.cardContent->width(), visibleHeight);
  }
  if (card.cardBackground != nullptr) {
    card.cardBackground->setFrameSize(card.cardBackground->width(), visibleHeight);
  }
  if (card.cardForeground != nullptr) {
    // The children keep their target layout while this node acts as the animated
    // viewport. This makes expansion a real height reveal instead of a content swap.
    card.cardForeground->setClipChildren(true);
    card.cardForeground->setFrameSize(card.cardForeground->width(), visibleHeight);
  }
}
...
InputArea* NotificationToast::buildCard(
  ...
  (void)chromeHosted;
  const float scale = notificationUiScale(m_config);
  const bool hasInlineReply = entry.expanded && hasInlineReplyAction(entry.actions);
  const bool showActions = entry.expanded && shouldShowNotificationActions(m_config);
  const float iconSize = notificationIconSize(scale, showActions);
  ...
  const float contentPad = (entry.expanded ? kCardInnerPad : 10.0f) * scale;

  auto viewport = std::make_unique<InputArea>();
  viewport->setAcceptedButtons(InputArea::buttonMask({BTN_LEFT, BTN_RIGHT}));
  viewport->setOnPress([this, id = entry.notificationId](const InputArea::PointerData& data) {
    auto* popup = findEntry(id);
    if (popup == nullptr || data.button != BTN_LEFT) {
      return;
    }
    if (data.pressed) {
      popup->gestureActive = true;
      popup->suppressNextClick = false;
      popup->gestureStartX = data.localX;
      popup->gestureStartY = data.localY;
      return;
    }
    if (!popup->gestureActive) {
      return;
    }
    popup->gestureActive = false;
    const float dx = data.localX - popup->gestureStartX;
    const float dy = data.localY - popup->gestureStartY;
    const float clearThreshold = m_config != nullptr ? m_config->config().notification.clearThreshold : 0.3f;
    const float expandThreshold = m_config != nullptr
        ? static_cast<float>(m_config->config().notification.expandThreshold)
        : 20.0f;
    const auto action = notification_card::classifyGesture(
        dx, dy, cardWidth(notificationUiScale(m_config)), popup->expanded, clearThreshold, expandThreshold
    );
    popup->suppressNextClick = action != notification_card::GestureAction::None
        || std::hypot(dx, dy) > 4.0f * notificationUiScale(m_config);
    if (action == notification_card::GestureAction::Dismiss) {
      DeferredCall::callLater([this, id]() { requestClose(id, CloseReason::Dismissed); });
    } else if (action == notification_card::GestureAction::Expand) {
      DeferredCall::callLater([this, id]() { setExpanded(id, true); });
    } else if (action == notification_card::GestureAction::Collapse) {
      DeferredCall::callLater([this, id]() { setExpanded(id, false); });
    }
  });
  viewport->setOnMotion([this, id = entry.notificationId](const InputArea::PointerData&) {
    if (auto* popup = findEntry(id); popup != nullptr && popup->gestureActive) {
      pauseTimeout(id);
    }
  });
  viewport->setOnClick([this, id = entry.notificationId,
    ...
    if (auto* popup = findEntry(id); popup != nullptr && popup->suppressNextClick) {
      popup->suppressNextClick = false;
      return;
    }
  ...
  *outCardForeground = foreground.get();
  ...
  if (entry.icon.has_value()) {
    ...
    if (rawIcon.size() > kGnilGlyphIconPrefix.size()
        && std::string_view(rawIcon.data(), kGnilGlyphIconPrefix.size()) == kGnilGlyphIconPrefix) {
      const std::string_view glyphName(
          rawIcon.data() + kGnilGlyphIconPrefix.size(), rawIcon.size() - kGnilGlyphIconPrefix.size()
      );
      if (!glyphName.empty()) {
        iconSlot->addChild(
            ui::glyph({
  ...
  if (!iconAssigned) {
    ...
    } else if (!entry.expanded && entry.imageData.has_value()) {
  ...
  auto textColumn = ui::column(
      ...
      ui::label({
          ...
          .color = colorSpecFromRole(ColorRole::OnSurface),
          .maxWidth = topTextMaxWidth,
          .maxLines = entry.expanded ? kMaxSummaryLines : 1,
      })
  );
...

  {
    ...
    const std::string relative = entry.receivedWallClock.has_value() ? formatTimeAgo(*entry.receivedWallClock)
                                                                     : formatElapsedSince(entry.receivedTime);
    if (!relative.empty()) {
      meta += meta.empty() ? relative : " · " + relative;
    }
  ...
  const float cardHeight = entry.expanded
      ? std::min(maxCardHeight, foreground->height())
      : static_cast<float>(kCompactCardHeight) * scale;
  cardRoot->setSize(cardW, cardHeight);
  viewport->setSize(cardW, cardHeight);
  // The outer viewport deliberately remains unclipped.  It owns the card
  // material and its antialiased border, while the layer-surface boundary
  // handles the off-screen slide.  A rectangular child clip here used to cut
  // the first card's top-left corner.

  cardRoot->addChild(
      ui::box({
          .out = outCardBackground,
          .width = cardW,
          .height = cardHeight,
          .configure = [scale, bgAlpha, critical = entry.urgency == Urgency::Critical](Box& box) {
            box.setRadius(Style::scaledRadiusLg(scale));
            box.setFill(
                colorSpecFromRole(critical ? ColorRole::Error : ColorRole::SurfaceVariant,
                                 critical ? 0.18f : bgAlpha)
            );
            box.setBorder(clearColorSpec(), 0.0f);
          },
      })
  );
  ...
  cardRoot->addChild(
      ui::button({
          .glyph = entry.expanded ? "expand_less" : "expand_more",
          .glyphSize = kCloseGlyphSize * scale,
          .variant = ButtonVariant::Ghost,
          .minWidth = closeButtonSize(scale),
          .minHeight = closeButtonSize(scale),
          .padding = 0.0f,
          .onClick = [this, id = entry.notificationId, expanded = entry.expanded]() { setExpanded(id, !expanded); },
          .onEnter = [this, notificationId = entry.notificationId]() { beginPopupHover(notificationId); },
          .onLeave = [this, notificationId = entry.notificationId,
                      totalDuration = entry.displayDurationMs]() { endPopupHover(notificationId, totalDuration); },
          .configure =
...
void NotificationToast::setExpanded(uint32_t notificationId, bool expanded) {
  auto* entry = findEntry(notificationId);
  if (entry == nullptr || entry->expanded == expanded || entry->exiting) {
    return;
  }

  const auto entryIt = std::ranges::find_if(m_entries, [notificationId](const PopupEntry& candidate) {
    return candidate.notificationId == notificationId;
  });
  if (entryIt == m_entries.end()) {
    return;
  }
  const std::size_t entryIndex = static_cast<std::size_t>(std::distance(m_entries.begin(), entryIt));
  entry->expanded = expanded;
  if (!expanded) {
    entry->replyInputFocused = false;
  }
  refreshEntryGeometry(*entry);

  // Commit the target stack geometry now. cardSurfaceY() substitutes each
  // instance's displayed card heights, so the visual stack still moves smoothly
  // from its current geometry while these shared coordinates describe the target.
  ...
  std::ranges::sort(placed, [this](std::size_t lhs, std::size_t rhs) {
    return m_entries[lhs].y < m_entries[rhs].y;
  });
  float cursor = paddingTop(m_config, notificationUiScale(m_config));
  const float gap = kGap * notificationUiScale(m_config);
  for (const std::size_t i : placed) {
    m_entries[i].y = cursor;
    cursor += m_entries[i].height + gap;
  }

  for (auto& instance : m_instances) {
    if (instance == nullptr || instance->sceneRoot == nullptr || entryIndex >= instance->cards.size()) {
      continue;
    }
    Instance& inst = *instance;
    auto& oldCard = inst.cards[entryIndex];
    if (oldCard.cardNode == nullptr) {
      continue;
    }

    const float fromHeight = std::max(1.0f, oldCard.clipHeight);
    const float fromY = oldCard.cardNode->y();
    const float foregroundOpacity = oldCard.cardForeground != nullptr ? oldCard.cardForeground->opacity() : 1.0f;
    if (oldCard.countdownAnimId != 0) inst.animations.cancel(oldCard.countdownAnimId);
    if (oldCard.entryAnimId != 0) inst.animations.cancel(oldCard.entryAnimId);
    if (oldCard.slideAnimId != 0) inst.animations.cancel(oldCard.slideAnimId);
    if (oldCard.exitAnimId != 0) inst.animations.cancel(oldCard.exitAnimId);
    if (oldCard.heightAnimId != 0) inst.animations.cancel(oldCard.heightAnimId);
    for (auto& sibling : inst.cards) {
      if (sibling.slideAnimId != 0) {
        inst.animations.cancel(sibling.slideAnimId);
        sibling.slideAnimId = 0;
      }
    }

    inst.sceneRoot->removeChild(oldCard.cardNode);
    oldCard = {};
    InputArea* rebuilt = buildCard(
        *entry, &oldCard.cardContent, &oldCard.cardForeground, &oldCard.cardBackground, &oldCard.progressBar,
        &oldCard.actionsRowNode, &oldCard.inlineReplyRowNode, &oldCard.inlineReplyInput, inst.chromeHosted
    );
    oldCard.cardNode = rebuilt;
    oldCard.targetHeight = rebuilt->height();
    if (oldCard.cardForeground != nullptr) {
      oldCard.cardForeground->setOpacity(foregroundOpacity);
    }
    setCardVisualHeight(oldCard, fromHeight);
    oldCard.cardNode->setPosition(horizontalInnerPad(notificationUiScale(m_config)), fromY);
    inst.sceneRoot->addChild(std::unique_ptr<Node>(rebuilt));

    if (std::abs(fromHeight - oldCard.targetHeight) <= 0.5f) {
      setCardVisualHeight(oldCard, oldCard.targetHeight);
      packCardsForDisplayedHeights(inst);
      updateInputRegion(inst);
      continue;
    }
    ...
    Instance* const instPtr = &inst;
    const float targetHeight = oldCard.targetHeight;
    oldCard.heightAnimId = inst.animations.animate(
        fromHeight, targetHeight, Style::animChromeSpatial, Easing::CaelestiaExpressiveSpatial,
        [this, instPtr, notificationId](float sampledHeight) {
          auto* card = findCardState(*instPtr, notificationId);
          if (card == nullptr) {
            return;
          }
          setCardVisualHeight(*card, sampledHeight);
          packCardsForDisplayedHeights(*instPtr);
          updateInputRegion(*instPtr);
          if (instPtr->surface != nullptr) {
            instPtr->surface->requestRedraw();
          }
        },
        [this, instPtr, notificationId, targetHeight]() {
          auto* card = findCardState(*instPtr, notificationId);
          if (card == nullptr) {
            return;
          }
          card->heightAnimId = 0;
          setCardVisualHeight(*card, targetHeight);
          packCardsForDisplayedHeights(*instPtr);
          updateInputRegion(*instPtr);
          if (instPtr->pointerInside) {
            instPtr->inputDispatcher.pointerMotion(instPtr->lastPointerX, instPtr->lastPointerY, 0);
          }
        },
        oldCard.cardNode
    );
...
void NotificationToast::enterInlineReplyMode(uint32_t notificationId) {
  for (auto& inst : m_instances) {
    ...
    for (std::size_t ci = 0; ci < inst->cards.size() && ci < m_entries.size(); ++ci) {
      ...
      if (inst->pointerInside && cs.inlineReplyInput != nullptr) {
        inst->inputDispatcher.setFocus(cs.inlineReplyInput->inputArea());
      }
      if (inst->surface != nullptr) {
        inst->surface->requestLayout();
      }
...
bool NotificationToast::exitInlineReplyMode(uint32_t notificationId) {
  auto* entry = findEntry(notificationId);
  if (entry == nullptr) {
    return false;
  }
  ...
  for (auto& inst : m_instances) {
    auto* state = findCardState(*inst, notificationId);
    if (state == nullptr || !state->replyMode) {
      continue;
    }
    changed = true;
    if (state->inlineReplyInput != nullptr
        && inst->inputDispatcher.focusedArea() == state->inlineReplyInput->inputArea()) {
      inst->inputDispatcher.setFocus(nullptr);
    }
    state->replyMode = false;
    if (state->actionsRowNode != nullptr) {
      state->actionsRowNode->setVisible(true);
      state->actionsRowNode->setParticipatesInLayout(true);
    }
    if (state->inlineReplyRowNode != nullptr) {
      state->inlineReplyRowNode->setVisible(false);
      state->inlineReplyRowNode->setParticipatesInLayout(false);
    }
    syncKeyboardInteractivity(*inst);
    if (inst->surface != nullptr) {
      inst->surface->requestLayout();
  ...
  if (changed) {
    entry->replyInputFocused = false;
    resumeTimeout(notificationId, entry->displayDurationMs);
  }
  return changed;
}
...
bool NotificationToast::onKeyboardEvent(const KeyboardEvent& event) {
  ...
  if (event.pressed && KeySymbol::isEscape(event.sym)) {
    for (std::size_t ci = 0; ci < target->cards.size() && ci < m_entries.size(); ++ci) {
      if (target->cards[ci].replyMode && exitInlineReplyMode(m_entries[ci].notificationId)) {
        return true;
      }
    }
    for (std::size_t ci = 0; ci < target->cards.size() && ci < m_entries.size(); ++ci) {
      if (target->cards[ci].cardNode != nullptr && m_entries[ci].expanded) {
        setExpanded(m_entries[ci].notificationId, false);
        return true;
      }
    }
  }
...
std::string NotificationToast::resolveNotificationIconPath(const PopupEntry& entry) {
  ...
  if (entry.icon->size() > kGnilGlyphIconPrefix.size()
      && std::string_view(entry.icon->data(), kGnilGlyphIconPrefix.size()) == kGnilGlyphIconPrefix) {
    return {};
  }
