...
namespace settings {
  ...
  void PluginStoreContent::buildGridView(Flex& body, Renderer& renderer, AsyncTextureCache* textureCache) {
    ...
    // Keep allocation in VirtualGridView's implementation unit. A second
    // allocation expression here can retain an older class size in an
    // incremental object and under-allocate the control.
    auto grid = ui::virtualGridView({
        .out = &m_grid,
        .minCellWidth = 200.0f * scale,
        .cellHeight = 260.0f * scale,
        .squareCells = false,
        .columnGap = Style::spaceSm * scale,
        .rowGap = Style::spaceSm * scale,
        .adapter = adapterPtr,
        // The sheet hosts the store without an outer ScrollView, so the
        // grid's own scroll fills the available height. A minimum height
        // would overflow shorter sheets.
        .flexGrow = 1.0f,
