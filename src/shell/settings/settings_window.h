...
private:
  ...
  NexusRoute* m_nexusRoute = nullptr;
  NexusHostCoordinator* m_nexusCoordinator = nullptr;
  std::unique_ptr<NexusView> m_nexusView;
  std::function<void(std::string, wl_output*)> m_openNexusPanel;
  ...
  // ConfigService invokes reload subscribers synchronously from a settings
  // write. Keep that echo from rebuilding the window a second time; the
  // mutation path patches the affected control directly.
