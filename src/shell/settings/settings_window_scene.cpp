...
namespace {
  ...
  std::optional<toml::table> configSectionTable(const Config& cfg, std::string_view section) {
    ...
    if (section == "sidebar") {
      return schema::writeTable(cfg.sidebar, schema::sidebarSchema());
    }
    if (section == "osd") {
...
void SettingsWindow::buildScene(std::uint32_t width, std::uint32_t height) {
  ...
  if (m_nexusView != nullptr) {
    ...
    m_sceneRoot = m_nexusView->build(
        ...
        [this]() {
          ...
          if (m_nexusCoordinator != nullptr) {
            (void)m_nexusCoordinator->activate(NexusHost::Panel, output);
          }
          close();
          if (m_openNexusPanel) {
            m_openNexusPanel(route, output);
          }
        },
        [this]() {
    ...
    m_inputDispatcher.setFocus(m_nexusView->initialFocusArea());
