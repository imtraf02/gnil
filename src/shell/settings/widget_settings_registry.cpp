...
namespace settings {
  ...
  std::vector<WidgetSettingSpec> widgetSettingSpecs(
    ...
    const std::vector<WidgetSettingSelectOption> workspaceDisplay = {
        {"state", "settings.widgets.options.state"},
    ...
    if (type == "active_window") {
      ...
      add(withGroup(boolSpec("show_windows", true), "workspaces.list"));
      add(withGroup(intSpec("max_window_icons", 5, 1.0, 10.0, 1.0), "workspaces.list"));
      {
