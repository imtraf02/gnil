...
namespace {
  ...
  constexpr float kWindowWidth = 1120.0f;
  constexpr float kWindowHeight = 630.0f;
...
void SettingsWindow::configureNexus(
    NexusRoute& route, NexusHostCoordinator& coordinator, NexusServices services,
    std::function<void(std::string, wl_output*)> openPanel
) {
  m_nexusRoute = &route;
  m_nexusCoordinator = &coordinator;
  m_nexusView = std::make_unique<NexusView>(route, services);
  m_openNexusPanel = std::move(openPanel);
}
...
void SettingsWindow::open(std::string context) {
  if (m_nexusRoute != nullptr && !context.empty()) {
    (void)m_nexusRoute->deepLink(context);
  }
  wl_output* routedOutput = m_nexusCoordinator != nullptr
      ? static_cast<wl_output*>(m_nexusCoordinator->output())
      : nullptr;
  if (m_nexusCoordinator != nullptr) {
    (void)m_nexusCoordinator->activate(NexusHost::Window, routedOutput);
  }
  if (!context.empty()) {
  ...
  wl_output* output = routedOutput != nullptr ? routedOutput : m_wayland->lastPointerOutput();
  if (output == nullptr) {
  ...
  m_surface->setUpdateCallback([this]() {
    if (m_nexusView != nullptr) {
      m_nexusView->update();
    }
  });
...
void SettingsWindow::destroyWindow() {
  ...
  m_surface.reset();
  if (m_nexusCoordinator != nullptr) {
    m_nexusCoordinator->release(NexusHost::Window);
    if (m_nexusCoordinator->host() == NexusHost::None && m_nexusView != nullptr) {
      m_nexusView->cancelTransientPrompts();
    }
  }
  m_pointerInside = false;
  m_output = nullptr;
...
void SettingsWindow::prepareFrame(bool /*needsUpdate*/, bool needsLayout) {
  ...
  if (needRebuild) {
    ...
    phaseProfileWatch.reset();
    if (m_nexusView != nullptr) {
      const bool preserveFocus = m_nexusView->contentRefreshPending();
      if (preserveFocus) {
        m_inputDispatcher.stashTabFocus();
      }
      m_nexusView->layout(*m_renderContext, w, h);
      if (preserveFocus) {
        m_inputDispatcher.restoreStashedTabFocus();
      }
    } else {
...
void SettingsWindow::requestUpdate() {
  if (isOpen() && m_surface != nullptr) {
