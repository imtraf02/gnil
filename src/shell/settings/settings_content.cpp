...
namespace settings {
  ...
  addSettingsContentSections(Flex& content, const std::vector<SettingEntry>& registry, SettingsContentContext ctx) {
    ...
    const auto entryPathKey = [](const SettingEntry& entry) {
      std::string key;
    ...
    for (const std::size_t entryIndex : entryOrder) {
      ...
      if (activeSection != nullptr) {
        ...
        if (!ctx.targetPath.empty() && entryPathKey(entry) == ctx.targetPath && ctx.setScrollTarget
            && !activeSection->children().empty()) {
          ctx.setScrollTarget(activeSection->children().back().get());
        }
        ++visibleEntries;
      }
    }
