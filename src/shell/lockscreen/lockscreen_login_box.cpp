...
namespace lockscreen_login_box {
  ...
  float defaultPanelHeight() { return 620.0f; }
  ...
  PanelContentLayout panelContentLayout(float panelWidth, float panelHeight, bool showLoginButton) {
    ...
    // The editor preview does not construct the live clock/profile subtree. Keep
    // its password ghost in the same lower portion occupied by the live control.
    layout.controlHeight = std::min(Style::controlHeight, panelHeight - Style::spaceSm * 2.0f);
    layout.contentTop = std::max(Style::spaceSm, panelHeight - layout.controlHeight - Style::spaceLg * 2.5f);
  ...
  void defaultPanelCenter(float screenWidth, float screenHeight, float& cx, float& cy) {
    ...
    const float panelY = std::max(Style::spaceLg, (screenHeight - height) * 0.5f);
  ...
  LoginBoxStyle resolveStyle(const std::unordered_map<std::string, WidgetSettingValue>& settings) {
    ...
    ColorSpec panelFill =
        colorSpecFromConfigString(readString(settings, "background_color", "surface"), "background_color");
  ...
  void applyDefaultSettings(
    ...
    if (scope == desktop_settings::DesktopWidgetSettingsScope::Widget) {
      ...
      settings.insert_or_assign(std::string(kInputOpacityKey), 0.92);
      settings.insert_or_assign(std::string(kInputRadiusKey), 22.0);
    ...
    if (scope == desktop_settings::DesktopWidgetSettingsScope::Background) {
      ...
      settings.insert_or_assign("background_opacity", 0.82);
      settings.insert_or_assign("background_radius", 36.0);
    }
  }
