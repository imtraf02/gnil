...
void LockScreen::unlock() {
  ...
  if (m_locked) {
    if (m_unlockAnimating) {
      return;
    }
    m_unlockAnimating = true;
    m_pendingUnlockAnimations = 0;
    invalidatePendingAuthentication();
    stopFingerprint();
    for (auto& instance : m_instances) {
      ...
      ++m_pendingUnlockAnimations;
      instance.surface->beginUnlockAnimation([this]() {
        if (--m_pendingUnlockAnimations == 0) {
          finishUnlock();
        }
      });
    }
...
void LockScreen::finishUnlock() {
  ...
  if (m_lock != nullptr) {
    if (m_locked) {
      ...
      kLog.info("session unlock requested after lockscreen exit animation");
    } else {
      ext_session_lock_v1_destroy(m_lock);
  ...
  m_desktopCaptures.clear();
  m_desktopCapturesPrimed = false;
  if (wasLockedInteractive && m_onSessionUnlocked) m_onSessionUnlocked();
  clearInstances();
  m_pointerSurface = nullptr;
  if (m_wayland != nullptr) wl_display_flush(m_wayland->display());
}
