...
void SessionPanel::create() {
  ...
  rootLayout->setColumnGap(Style::spaceLg * scale);
  rootLayout->setRowGap(Style::spaceLg * scale);
  ...
  for (std::size_t i = 0; i < m_visibleEntries.size(); ++i) {
    ...
    // Shortcuts remain active, but Caelestia's compact session rail does not
    // render key badges over the action icons.
...
Button* SessionPanel::createActionButton(const SessionPanelActionConfig& cfg, std::size_t index, float scale) {
  ...
  // Caelestia's right-edge session drawer is an icon-only vertical rail. The
  // configured label remains available to the action model, but rendering it
  // here makes the rail too wide and prevents it reading as part of the frame.
  button->setText("");
  button->setBadge("");
  button->setGlyph(
  ...
  button->setGlyphSize(32.0f * scale);
