...
namespace {
  ...
  float naturalCalendarDayCellHeight(float innerWidth, float scale) {
    const float dayColumnWidth = std::max(0.0f, (innerWidth - kCalendarGridGap * scale * 6.0f) / 7.0f);
    return std::clamp(dayColumnWidth, kCalendarCellSizeMin * scale, kCalendarCellSizeMax * scale);
  }
...
std::unique_ptr<Flex> CalendarTab::create() {
  ...
  if (m_config != nullptr) {
    m_showEventsCard = m_config->config().calendar.showEventsCard;
  }
...
void CalendarTab::doPrepareIntrinsicLayout(Renderer& renderer, float contentWidth, float maxBodyHeight) {
  (void)maxBodyHeight;
  if (m_card == nullptr || m_calendarArea == nullptr) {
    return;
  }
  const float innerWidth = std::max(0.0f, contentWidth - (m_card->paddingLeft() + m_card->paddingRight()));
  syncCalendarStructure(renderer, innerWidth);
  auto constraints = LayoutConstraints::unconstrained();
  constraints.setExactWidth(contentWidth);
  const float naturalHeight = m_card->measureIntrinsic(renderer, constraints).height;
  if (std::isfinite(naturalHeight) && naturalHeight > 0.0f) {
    // InputArea deliberately has no layout semantics, so publish the measured
    // child extent on its wrapper for the parent Flex intrinsic pass.
    m_calendarArea->setSize(contentWidth, naturalHeight);
  }
}
...
void CalendarTab::syncCalendarStructure(Renderer& renderer, float innerWidth) {
  ...
  m_lastInnerHeight = naturalCalendarDayCellHeight(innerWidth, contentScale());
  ...
  rebuild(innerWidth, m_lastInnerHeight);
  if (m_grid != nullptr) {
    if (m_monthSlideAnimId == 0 && !m_startMonthSlideIn) {
...
void CalendarTab::rebuild(float innerWidth, float dayCellHeight) {
  ...
  const float dayButtonSize = std::round(
      std::min({dayCellHeight, dayColumnWidth, kCalendarDayButtonSizeMax * scale})
  );
  ...
  if (m_gridViewport != nullptr) {
    m_gridViewport->setMinHeight(gridContentHeight);
...
void CalendarTab::toggleEventsCard() {
  ...
  if (m_config != nullptr) {
    m_config->setOverride({"calendar", "show_events_card"}, m_showEventsCard);
  }
