...
PanelPlacement ControlCenterPanel::panelPlacement() const noexcept {
  // The dashboard owns a fixed top-centre slot in GNIL's chrome.  Letting an
  // old attached-panel setting pull it toward the left rail breaks that
  // spatial contract, so legacy placement values are intentionally ignored.
...
void ControlCenterPanel::layoutDashboardTabIndicator() {
  if (m_dashboardTabIndicator == nullptr || m_dashboardTabStrip == nullptr || m_dashboardTabSlots.empty()) {
    return;
  }

  std::size_t selectedSlot = 0;
  for (std::size_t i = 0; i < m_dashboardTabSlots.size(); ++i) {
    if (m_dashboardTabSlots[i] == m_activeTab) {
      selectedSlot = i;
      break;
    }
  }

  const float stripWidth = std::max(0.0f, m_dashboardTabStrip->width());
  const float slotWidth = stripWidth / static_cast<float>(m_dashboardTabSlots.size());
  const float thickness = std::max(3.0f, Style::borderWidth * 3.0f * contentScale());
  m_dashboardTabIndicator->setPosition(
      m_dashboardTabStrip->x() + slotWidth * static_cast<float>(selectedSlot),
      m_dashboardTabStrip->y() + std::max(0.0f, m_dashboardTabStrip->height() - thickness)
  );
  m_dashboardTabIndicator->setSize(slotWidth, thickness);
}

void ControlCenterPanel::syncDashboardVisualSize(bool animate) {
  if (m_showSidebar) {
    PanelManager::instance().requestActivePanelVisualSize(preferredWidth(), preferredHeight(), animate);
    return;
  }

  // Caelestia sizes the drawer from the active tab's implicit content. These
  // deterministic fallbacks preserve that spatial rhythm before a tab has had
  // a layout pass, while the panel manager smoothly morphs the visible body.
  float width = 854.0f;
  float height = 520.0f;
  switch (m_activeTab) {
  case TabId::Media:
...
void ControlCenterPanel::doUpdate(Renderer& renderer) {
  ...
  if (m_rootLayout != nullptr) {
    const float targetWidth = preferredWidth();
    LayoutConstraints constraints;
    constraints.minWidth = targetWidth;
    constraints.maxWidth = targetWidth;
    constraints.hasMaxWidth = true;
    constraints.hasMaxHeight = false;

    LayoutSize measuredSize = m_rootLayout->measure(renderer, constraints);

    const float scale = contentScale();
    const float minH = 320.0f * scale;
    const float maxH = 640.0f * scale;
    const float finalHeight = std::clamp(measuredSize.height, minH, maxH);

    if (std::abs(finalHeight - m_lastRequestedHeight) > 1.0f) {
      m_lastRequestedHeight = finalHeight;
      PanelManager::instance().requestActivePanelVisualSize(targetWidth, finalHeight, true);
    }
  }
}
...
void ControlCenterPanel::onClose() {
  ...
  m_content = nullptr;
  m_dashboardTabStrip = nullptr;
  m_dashboardTabIndicator = nullptr;
...
void ControlCenterPanel::selectTab(TabId tab, bool animated) {
  ...
  syncDashboardVisualSize(tabChanged && animated);
}
...
ControlCenterSidebarMode ControlCenterPanel::sidebarModeForOpen(std::string_view context) const {
  // The four-tab dashboard is deliberately small. A bar widget that opens a
  // technical destination (audio/network/bluetooth…) keeps the existing
  // compact navigator instead of pretending that destination is a dashboard
  // tab with the wrong active indicator.
  const bool dashboardContext = context == "media" || context == "system" || context == "weather";
  if (isDirectSectionOpenContext(context) && !dashboardContext) {
