...
public:
  ...
  [[nodiscard]] float preferredWidth() const override { return scaled(360.0f); }
  [[nodiscard]] float preferredHeight() const override { return scaled(9999.0f); }
...
private:
  ...
  // directory. Any mutation must be followed by rebindGrid(), which rebuilds
  // the three physical strips consumed by the virtual grid adapter.
  std::vector<WallpaperEntry> m_visibleEntries;
  // Three identical physical strips let navigation cross either logical edge
  // before an invisible rebase returns the viewport to the middle strip.
  std::vector<WallpaperEntry> m_physicalEntries;
  ...
  std::size_t m_cycleMoveSerial = 0;
