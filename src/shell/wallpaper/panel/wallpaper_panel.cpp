...
public:
  using ActivateCallback = std::function<void(std::size_t, const WallpaperEntry&)>;
  ...
  void onActivate(std::size_t index) override {
    ...
    m_onActivate(index, (*m_entries)[index]);
  }
...
WallpaperPanel::WallpaperPanel(
    ...
    : m_wayland(wayland), m_config(config), m_thumbnails(thumbnails), m_scanner(scanner), m_wallpaper(wallpaper) {
  if (m_config != nullptr) {
...
void WallpaperPanel::create() {
  ...
  m_adapter->setEntries(&m_physicalEntries);
  m_adapter->setOnActivate([this](std::size_t physicalIndex, const WallpaperEntry& entry) {
    if (!m_visibleEntries.empty()) {
      (void)m_cycle.selectPhysical(physicalIndex);
      m_selectedVisibleIndex = m_cycle.logicalIndex();
    }
    if (entry.isDir) {
      ...
      ui::virtualGridView({
          ...
          .onSelectionChanged =
              [this](std::optional<std::size_t> idx) {
                ...
                if (idx.has_value() && *idx < m_physicalEntries.size() && !m_visibleEntries.empty()) {
                  (void)m_cycle.selectPhysical(*idx);
                  m_selectedVisibleIndex = m_cycle.logicalIndex();
                } else {
  ...
  if (m_grid != nullptr && m_grid->focusArea() != nullptr) {
    m_grid->focusArea()->setOnFocusGain([this]() {
      ...
      if (m_grid != nullptr && hasVisibleSelection()) {
        ...
        m_grid->setSelectedIndex(m_cycle.physicalIndex());
  ...
  // The panel is intentionally a one-column picker: drag scrolling is off and
  // every wheel event advances exactly one physical item in the cyclic strip.
  if (m_grid != nullptr) {
    m_grid->scrollView().setScrollbarVisible(false);
    m_grid->scrollView().setDragScrollingEnabled(false);
    m_grid->scrollView().setWheelHandler([this](float delta) {
      stepCyclicSelection(delta > 0.0f ? 1 : -1);
      return true;
    });
  }
...
void WallpaperPanel::doLayout(Renderer& renderer, float width, float height) {
  ...
  if (m_grid != nullptr) {
    const float scale = contentScale();
    const float innerWidth = std::max(100.0f, width - Style::cardPadding * 2.0f * scale);
    const float labelH = Style::fontSizeCaption * scale * 1.4f;
    const float innerGap = Style::spaceXs * scale;
    const float cellHeight = innerWidth * 0.5625f + Style::spaceXs * 2.0f * scale + innerGap + labelH;
...
void WallpaperPanel::onOpen(std::string_view /*context*/) {
  ...
  // Select the current active wallpaper on open so the user can immediately use keyboard arrow keys!
  std::size_t initialSelection = kNoVisibleSelection;
  const std::string currentPath = currentWallpaperPathForSelection();
  for (std::size_t i = 0; i < m_visibleEntries.size(); ++i) {
    if (wallpaperEntryIsCurrent(m_visibleEntries[i], currentPath)) {
      initialSelection = i;
      break;
    }
  }
  if (initialSelection != kNoVisibleSelection) {
    selectVisibleIndex(initialSelection);
  } else if (!m_visibleEntries.empty()) {
...
bool WallpaperPanel::handleGlobalKey(std::uint32_t sym, std::uint32_t modifiers, bool pressed, bool preedit) {
  ...
  if (focused != nullptr) {
    const bool onInput = (m_filterInput != nullptr && focused == m_filterInput->inputArea());
    const bool inGrid = (m_grid != nullptr && isDescendantOf(focused, m_grid));
    if (!onInput && !inGrid) {
      return false;
    }
    if (onInput) {
...
void WallpaperPanel::onScanComplete() {
  ...
  applyFilter();
  if (m_selectedVisibleIndex == kNoVisibleSelection && !m_visibleEntries.empty()) {
    std::size_t initialSelection = kNoVisibleSelection;
    const std::string currentPath = currentWallpaperPathForSelection();
    for (std::size_t i = 0; i < m_visibleEntries.size(); ++i) {
      if (wallpaperEntryIsCurrent(m_visibleEntries[i], currentPath)) {
...
void WallpaperPanel::rebuildPhysicalEntries() {
  const std::size_t logicalCount = m_visibleEntries.size();
  m_cycle.setCount(logicalCount);
  if (m_selectedVisibleIndex < logicalCount) {
    (void)m_cycle.select(m_selectedVisibleIndex);
  } else if (logicalCount > 0) {
    (void)m_cycle.select(0);
  }

  m_physicalEntries.clear();
  m_physicalEntries.reserve(logicalCount * 3);
  for (int strip = 0; strip < 3; ++strip) {
    m_physicalEntries.insert(m_physicalEntries.end(), m_visibleEntries.begin(), m_visibleEntries.end());
  }
}
...
void WallpaperPanel::rebindGrid(bool resetScroll) {
  ...
  if (m_visibleEntries.empty() || !hasVisibleSelection() || !m_gridKeyboardActive) {
    m_grid->setSelectedIndex(std::nullopt, false);
    if (!m_visibleEntries.empty()) {
      m_grid->scrollToIndex(m_cycle.physicalIndex());
    }
  } else {
    m_grid->setSelectedIndex(m_cycle.physicalIndex());
  }
...
void WallpaperPanel::applyWallpaperPath(const std::string& path, const WallpaperFavorite* applyTheme) {
  ...
  // A static choice must visibly take ownership back from native video
  // playback before the image/palette selection is persisted.
  if (m_wallpaper != nullptr) {
    (void)m_wallpaper->clearVideoWallpaper(connector);
  }
...
void WallpaperPanel::selectVisibleIndex(std::size_t index) {
  ...
  // Direct selection starts in the middle strip. Directional movement is the
  // only path allowed to enter an outer copy.
  m_cycle.setCount(m_visibleEntries.size());
  (void)m_cycle.select(index);
  if (m_grid != nullptr) {
    m_syncingGridSelectionVisual = true;
    m_grid->setSelectedIndex(m_cycle.physicalIndex(), false);
    m_grid->scrollToIndex(m_cycle.physicalIndex());
    m_syncingGridSelectionVisual = false;
  }
  syncThemeControls();
...
void WallpaperPanel::stepCyclicSelection(int direction) {
  if (m_visibleEntries.empty() || direction == 0 || m_grid == nullptr) {
    return;
  }

  if (m_cycle.count() != m_visibleEntries.size()) {
    rebuildPhysicalEntries();
    m_grid->notifyDataChanged();
  }

  // A clicked outer-strip copy is stationary, so normalize it before the next
  // movement while retaining the exact viewport-relative position.
  const std::size_t beforeNormalize = m_cycle.physicalIndex();
  if (m_cycle.rebase()) {
    const std::size_t afterNormalize = m_cycle.physicalIndex();
    m_syncingGridSelectionVisual = true;
    m_grid->setSelectedIndex(afterNormalize, false);
    m_syncingGridSelectionVisual = false;
    m_grid->rebaseIndex(beforeNormalize, afterNormalize);
  }

  if (!hasVisibleSelection()) {
    (void)m_cycle.select(0);
  } else {
    (void)m_cycle.move(direction > 0 ? 1 : -1);
  }
  m_selectedVisibleIndex = m_cycle.logicalIndex();
  ...
  const std::size_t serial = ++m_cycleMoveSerial;
  if (!m_grid->animateToIndex(
          m_cycle.physicalIndex(), [this, serial]() { finishCyclicMovement(serial); }
      )) {
    // Input normally starts after the first layout. If it arrives earlier,
    // preserve the requested item and let the next movement normalize the
    // physical strip once metrics are available.
    m_grid->scrollToIndex(m_cycle.physicalIndex());
  }
  syncThemeControls();
  m_dirty = true;
...
void WallpaperPanel::finishCyclicMovement(std::size_t serial) {
  ...
  const std::size_t afterRebase = m_cycle.physicalIndex();
  m_syncingGridSelectionVisual = true;
  m_grid->setSelectedIndex(afterRebase, false);
  m_syncingGridSelectionVisual = false;
  m_grid->rebaseIndex(beforeRebase, afterRebase);
}
