...
namespace {
  ...
  [[nodiscard]] std::optional<std::string>
  resolveVideoWallpaperPath(std::string_view path, std::optional<std::string_view> callerCwd = std::nullopt) {
    if (path.empty()) {
      return std::nullopt;
    }
    const std::filesystem::path resolved = FileUtils::resolvePath(path, callerCwd);
  ...
  [[nodiscard]] const VideoWallpaperOutput*
  videoWallpaperForOutput(const WallpaperConfig& config, const WaylandOutput& output) {
    const VideoWallpaperOutput* fallback = nullptr;
    for (const auto& assignment : config.videoOutputs) {
      if (assignment.match == output.connectorName) {
        return &assignment;
      }
      if (assignment.match == "all" || assignment.match == "*") {
        fallback = &assignment;
      } else if (outputMatchesSelector(assignment.match, output)) {
        return &assignment;
      }
    }
    return fallback;
  ...
  [[nodiscard]] std::string videoMpvOptions(const VideoWallpaperOutput& assignment) {
    std::string options = "loop-file=inf panscan=1.0 ";
    options += assignment.mute ? "no-audio" : "audio=yes";
    options += assignment.hardwareDecode ? " hwdec=auto" : " hwdec=no";
    if (assignment.keepLastFrame) {
      options += " keep-open=yes";
    }
    if (!assignment.mpvOptions.empty()) {
      options += ' ' + assignment.mpvOptions;
    }
    return options;
  }
...
struct Wallpaper::LivePaletteAsyncGuard {
  std::mutex mutex;
  Wallpaper* owner = nullptr;
  std::uint64_t generation = 0;
};

Wallpaper::Wallpaper() : m_livePaletteAsyncGuard(std::make_shared<LivePaletteAsyncGuard>()) {
  m_livePaletteAsyncGuard->owner = this;
}

Wallpaper::~Wallpaper() {
  {
    std::lock_guard lock(m_livePaletteAsyncGuard->mutex);
    m_livePaletteAsyncGuard->owner = nullptr;
    ++m_livePaletteAsyncGuard->generation;
  }
  std::vector<std::string> videoConnectors;
  videoConnectors.reserve(m_videoProcesses.size());
  for (const auto& [connector, process] : m_videoProcesses) {
    (void)process;
    videoConnectors.push_back(connector);
  }
  for (const auto& connector : videoConnectors) {
    stopVideoWallpaper(connector, false);
  }
...
bool Wallpaper::applyWallpaperImage(const std::optional<std::string>& connector, const std::string& path) {
  ...
  // Choosing a static wallpaper intentionally hands the output back from the
  // native video controller before persisting the image.
  (void)clearVideoWallpaper(connector);
  applyResolvedWallpaper(connector, *resolved);
  return true;
}

bool Wallpaper::videoWallpaperAvailable() const noexcept {
  return process::commandExists("mpvpaper") && process::commandExists("mpv");
}

bool Wallpaper::applyVideoWallpaper(const std::optional<std::string>& connector, const std::string& path) {
  if (m_config == nullptr || (connector.has_value() && !isConnectorKnown(*connector))) {
    return false;
  }
  const auto resolved = resolveVideoWallpaperPath(path);
  if (!resolved.has_value()) {
    return false;
  }
  ...
  if (connector.has_value()) {
    targets.push_back(*connector);
  } else if (m_wayland != nullptr) {
    for (const auto& output : m_wayland->outputs()) {
      if (output.done && output.hasUsableGeometry() && !output.connectorName.empty()) {
        targets.push_back(output.connectorName);
      }
    }
  }
  if (targets.empty()) {
  ...
  changes.reserve(targets.size() * 2);
  for (const auto& target : targets) {
    changes.emplace_back(std::vector<std::string>{"wallpaper", "video", target, "path"}, *resolved);
    changes.emplace_back(std::vector<std::string>{"wallpaper", "video", target, "enabled"}, true);
  }
  return m_config->setOverrides(std::move(changes));
}

bool Wallpaper::clearVideoWallpaper(const std::optional<std::string>& connector) {
  if (m_config == nullptr || (connector.has_value() && !isConnectorKnown(*connector))) {
    return false;
  }
  std::vector<std::string> targets;
  if (connector.has_value()) {
    ...
    for (const auto& [target, process] : m_videoProcesses) {
      (void)process;
      targets.push_back(target);
    }
    if (targets.empty() && m_wayland != nullptr) {
      for (const auto& output : m_wayland->outputs()) {
  ...
  for (const auto& target : targets) {
    changes.emplace_back(std::vector<std::string>{"wallpaper", "video", target, "enabled"}, false);
  }
  return m_config->setOverrides(std::move(changes));
}

void Wallpaper::stopVideoWallpaper(const std::string& connector, bool restoreImage) {
  const auto it = m_videoProcesses.find(connector);
  if (it == m_videoProcesses.end()) {
    return;
  }
  if (it->second.pid > 0) {
    process::terminateTracked(it->second.pid);
  }
  m_videoProcesses.erase(it);
  setOutputExternallyManaged(connector, false);
  if (restoreImage && m_wallpaperEnabled) {
    syncInstances();
  }
}

void Wallpaper::syncVideoWallpapers() {
  if (m_config == nullptr || m_wayland == nullptr) {
    return;
  }

  const auto& config = m_config->config().wallpaper;
  std::unordered_map<std::string, const VideoWallpaperOutput*> desired;
  for (const auto& output : m_wayland->outputs()) {
    if (!output.done || !output.hasUsableGeometry() || output.connectorName.empty()) {
      continue;
    }
    if (const auto* assignment = videoWallpaperForOutput(config, output);
        assignment != nullptr && assignment->enabled && !assignment->path.empty()) {
      desired.emplace(output.connectorName, assignment);
    }
  }

  std::vector<std::string> stale;
  for (const auto& [connector, running] : m_videoProcesses) {
    const auto desiredIt = desired.find(connector);
    if (desiredIt == desired.end() || desiredIt->second->path != running.path
        || videoMpvOptions(*desiredIt->second) != running.options) {
      stale.push_back(connector);
    }
  }
  for (const auto& connector : stale) {
    stopVideoWallpaper(connector, true);
  }

  if (!videoWallpaperAvailable()) {
    if (!desired.empty()) {
      kLog.warn("video wallpaper is configured but mpvpaper and mpv are not available");
    }
    syncLiveWallpaperPalette();
    return;
  }

  for (const auto& [connector, assignment] : desired) {
    if (m_videoProcesses.contains(connector)) {
      continue;
    }
    const auto resolved = resolveVideoWallpaperPath(assignment->path);
    if (!resolved.has_value()) {
      kLog.warn("video wallpaper for {} does not exist: {}", connector, assignment->path);
      continue;
    }
    const std::string options = videoMpvOptions(*assignment);
    std::vector<std::string> args = {"mpvpaper"};
    if (assignment->autoPause) {
      args.push_back("--auto-pause");
    }
    args.push_back("-o");
    args.push_back(options);
    args.push_back(connector);
    args.push_back(*resolved);
    const auto pid = process::launchDetachedTracked(args);
    if (!pid.has_value()) {
      kLog.warn("failed to launch mpvpaper for {}", connector);
      continue;
    }
    m_videoProcesses.emplace(connector, VideoProcess{.pid = *pid, .path = *resolved, .options = options});
    setOutputExternallyManaged(connector, true);
  }
  syncLiveWallpaperPalette();
}

std::optional<std::pair<std::string, std::string>>
Wallpaper::selectLiveWallpaperVideo(std::string_view outputSelector) const {
  if (m_wayland == nullptr || m_config == nullptr) {
    return std::nullopt;
  }

  const auto activeForOutput = [this](const WaylandOutput& output)
      -> std::optional<std::pair<std::string, std::string>> {
    const auto running = m_videoProcesses.find(output.connectorName);
    if (running == m_videoProcesses.end() || running->second.path.empty()) {
      return std::nullopt;
    }
    return std::pair{running->second.path, output.connectorName};
  };

  if (!outputSelector.empty() && outputSelector != "auto") {
    for (const auto& output : m_wayland->outputs()) {
      if (!output.done || !output.hasUsableGeometry()) {
        continue;
      }
      if (output.connectorName == outputSelector || outputMatchesSelector(std::string(outputSelector), output)) {
        return activeForOutput(output);
      }
    }
    return std::nullopt;
  }

  // A global assignment is the least surprising auto source on multi-monitor
  // setups, even if another monitor also has a more specific override.
  const auto& wallpaper = m_config->config().wallpaper;
  for (const auto& assignment : wallpaper.videoOutputs) {
    if ((assignment.match != "all" && assignment.match != "*") || !assignment.enabled || assignment.path.empty()) {
      continue;
    }
    const auto globalPath = resolveVideoWallpaperPath(assignment.path);
    if (!globalPath.has_value()) {
      break;
    }
    for (const auto& output : m_wayland->outputs()) {
      if (auto active = activeForOutput(output); active.has_value() && active->first == *globalPath) {
        return active;
      }
    }
    break;
  ...
  for (const auto& output : m_wayland->outputs()) {
    if (output.done && output.hasUsableGeometry()) {
      if (auto active = activeForOutput(output)) {
        return active;
      }
    }
  }
...
void Wallpaper::syncLiveWallpaperPalette() {
  ...
  ensureLiveWallpaperPalette(m_config->config().theme.liveWallpaperOutput);
  const std::string currentIdentity = m_livePaletteSource.has_value() ? m_livePaletteSource->identity : std::string{};
...
void Wallpaper::ensureLiveWallpaperPalette(std::string_view outputSelector) {
  const auto selected = selectLiveWallpaperVideo(outputSelector);
  ...
  std::error_code ec;
  const auto canonical = std::filesystem::weakly_canonical(selected->first, ec);
  const std::string path = ec ? selected->first : canonical.string();
  ec.clear();
  const auto modified = std::filesystem::last_write_time(path, ec);
  const auto mtime = ec ? 0 : modified.time_since_epoch().count();
  ec.clear();
  const auto size = std::filesystem::file_size(path, ec);
  const auto safeSize = ec ? 0 : size;
  const std::string identity =
      path + "\n" + std::to_string(mtime) + "\n" + std::to_string(safeSize) + "\n" + selected->second
      + "\n" + std::string(outputSelector);
  if (identity == m_livePaletteDesiredIdentity) {
    return;
  }

  m_livePaletteDesiredIdentity = identity;
  m_livePaletteSource.reset();
  const std::uint64_t generation = ++m_livePaletteGeneration;
  {
    std::lock_guard lock(m_livePaletteAsyncGuard->mutex);
    m_livePaletteAsyncGuard->generation = generation;
  }

  const std::string base = FileUtils::cacheDir();
  if (base.empty()) {
    kLog.warn("cannot cache live wallpaper palette frames: cache directory is unavailable");
    return;
  }
  ...
  for (std::size_t i = 0; i < 4; ++i) {
    const auto directory = cacheRoot / ("frame-" + std::to_string(i));
    std::filesystem::create_directories(directory, ec);
    const auto frame = directory / "00000001.png";
    framePaths.push_back(frame.string());
    ec.clear();
    const bool regular = std::filesystem::is_regular_file(frame, ec);
    ec.clear();
    const auto frameSize = regular ? std::filesystem::file_size(frame, ec) : 0;
    if (!regular || ec || frameSize == 0) {
      missing.push_back(i);
    }
  }
  if (missing.empty()) {
  ...
  if (!process::commandExists("mpv")) {
    kLog.warn("live wallpaper palette requested but mpv is unavailable");
    return;
  }

  struct ExtractionJob {
    std::weak_ptr<LivePaletteAsyncGuard> guard;
    std::uint64_t generation = 0;
    std::string identity;
    std::vector<std::string> framePaths;
  ...
  job->guard = m_livePaletteAsyncGuard;
  job->generation = generation;
  job->identity = identity;
  job->framePaths = framePaths;
  job->pending = missing.size();
  static constexpr std::string_view starts[] = {"12.5%", "37.5%", "62.5%", "87.5%"};

  const auto finishOne = [job]() {
    if (job->pending.fetch_sub(1) != 1) {
      return;
    }
    DeferredCall::callLater([job]() {
      auto guard = job->guard.lock();
      if (!guard) {
        return;
      }
      Wallpaper* owner = nullptr;
      {
        ...
        if (guard->owner == nullptr || guard->generation != job->generation) {
          return;
        }
        owner = guard->owner;
      }
      owner->completeLivePaletteExtraction(job->generation, job->identity, std::move(job->framePaths));
    });
  };

  for (const std::size_t index : missing) {
    const auto outDirectory = cacheRoot / ("frame-" + std::to_string(index));
    std::vector<std::string> args = {
        "mpv", "--no-config", "--no-audio", "--really-quiet", "--start=" + std::string(starts[index]),
        "--frames=1", "--vo=image", "--vo-image-format=png", "--vo-image-outdir=" + outDirectory.string(), path,
    };
    process::RunCallbacks callbacks;
    callbacks.onExit = [finishOne, framePath = framePaths[index]](process::RunResult result) {
      if (!result) {
    ...
    if (!process::runAsync(args, std::move(callbacks), std::move(options))) {
      finishOne();
    }
  }
}

void Wallpaper::completeLivePaletteExtraction(
    std::uint64_t generation, std::string identity, std::vector<std::string> framePaths
) {
  if (generation != m_livePaletteGeneration || identity != m_livePaletteDesiredIdentity) {
    return;
  }
  std::erase_if(framePaths, [](const std::string& path) {
    std::error_code ec;
  ...
  if (framePaths.empty()) {
    kLog.warn("mpv could not extract a palette sample from the live wallpaper");
    m_livePaletteDesiredIdentity.clear();
    return;
  }
  m_livePaletteSource = gnil::theme::LiveWallpaperPaletteSource{
...
bool Wallpaper::previewWallpaperImage(const std::optional<std::string>& connector, const std::string& path) {
  if (m_config == nullptr || !m_wallpaperEnabled) {
    return false;
  }
  const auto resolved = resolveWallpaperPath(path);
  if (!resolved.has_value() || (connector.has_value() && !isConnectorKnown(*connector))) {
    return false;
  }

  bool applied = false;
  for (auto& instance : m_instances) {
    if (connector.has_value() && instance->connectorName != *connector) {
      continue;
    }
    loadWallpaper(*instance, *resolved);
    m_previewedConnectors.insert(instance->connectorName);
    applied = true;
  }
  m_previewActive = applied;
  return applied;
}

void Wallpaper::restorePersistedWallpaper(WallpaperInstance& instance) {
  if (m_config == nullptr) {
    return;
  }
  const std::string persisted = m_config->getWallpaperPath(instance.connectorName);
  if (!persisted.empty() && persisted != instance.currentPath) {
    loadWallpaper(instance, persisted);
  }
}

void Wallpaper::clearPreview() {
  if (!m_previewActive) {
    return;
  }
  for (auto& instance : m_instances) {
    if (m_previewedConnectors.contains(instance->connectorName)) {
      restorePersistedWallpaper(*instance);
    }
  }
  m_previewedConnectors.clear();
  m_previewActive = false;
}

void Wallpaper::commitPreview() {
  m_previewedConnectors.clear();
  m_previewActive = false;
}
...
void Wallpaper::registerIpc(IpcService& ipc) {
  ...
  ipc.registerHandler(
      ...
      [this, &ipc, validateOutputConnector](const std::string& args) -> std::string {
        const auto tokens = StringUtils::splitWhitespace(StringUtils::trim(args));
        if (tokens.empty()) {
          return "error: path required (video-wallpaper-set [<connector>] <path>)\n";
        }
        const std::optional<std::string_view> callerCwd =
            ipc.callerCwd().has_value() ? std::optional<std::string_view>{*ipc.callerCwd()} : std::nullopt;
        const auto parsed = parseWallpaperSetTokens(
            tokens, [&](const std::string& value) { return validateOutputConnector(value).empty(); }, callerCwd
        );
        if (parsed.path.empty()) {
          return "error: path required (video-wallpaper-set [<connector>] <path>)\n";
        }
        if (parsed.connector.has_value()) {
          if (const std::string error = validateOutputConnector(*parsed.connector); !error.empty()) {
            return error;
          }
        }
        const auto resolved = resolveVideoWallpaperPath(parsed.path, callerCwd);
        if (!resolved.has_value()) {
          return "error: video path does not exist or is not a regular file\n";
        }
        if (!videoWallpaperAvailable()) {
          return "error: video wallpaper requires mpvpaper and mpv\n";
        }
        return applyVideoWallpaper(parsed.connector, *resolved) ? "ok\n" : "error: could not save video wallpaper\n";
      },
      "video-wallpaper-set [<connector>] <path>", "Set a persisted video wallpaper"
  );
  ipc.registerHandler(
      "video-wallpaper-clear",
      [this, validateOutputConnector](const std::string& args) -> std::string {
        const std::string connector = StringUtils::trim(args);
        if (!connector.empty()) {
          if (const std::string error = validateOutputConnector(connector); !error.empty()) {
            return error;
          }
        }
        return clearVideoWallpaper(connector.empty() ? std::nullopt : std::optional<std::string>{connector})
            ? "ok\n"
            : "error: could not clear video wallpaper\n";
      },
      "video-wallpaper-clear [<connector>]", "Stop a video wallpaper and restore the image wallpaper"
  );
}
...
void Wallpaper::syncInstances() {
  ...
  std::erase_if(m_instances, [&](const auto& inst) {
    ...
    // The native video wallpaper controller now owns this output
