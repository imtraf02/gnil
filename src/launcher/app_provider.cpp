...
namespace {
  ...
  bool configuredAppContains(const std::vector<std::string>& configured, const DesktopEntry& entry) {
    return std::ranges::any_of(configured, [&entry](const std::string& value) {
      return value == entry.id || value == entry.path || StringUtils::toLower(value) == entry.idLower;
    });
  }
...
std::vector<LauncherResult> AppProvider::query(std::string_view text) const {
  ...
  const auto* launcher = m_config != nullptr ? &m_config->config().shell.launcher : nullptr;
  const auto hidden = [launcher](const DesktopEntry& entry) {
    return launcher != nullptr && configuredAppContains(launcher->hiddenApps, entry);
  };
  const auto favourite = [launcher](const DesktopEntry& entry) {
    return launcher != nullptr && configuredAppContains(launcher->favouriteApps, entry);
  };
  ...
  if (pattern.empty()) {
    ...
    for (const auto& entry : m_entries) {
      if (!hidden(entry)) {
        results.push_back(buildResult(entry, favourite(entry) ? 10'000.0 : 0.0));
      }
    }
    std::ranges::stable_sort(results, std::greater{}, &LauncherResult::score);
    return results;
  }
  ...
  for (const auto& entry : m_entries) {
    ...
    const double baseScore = scoreEntry(pattern, entry);
    if (FuzzyMatch::isMatch(baseScore)) {
      const double s = baseScore + (favourite(entry) ? 10'000.0 : 0.0);
