...
ScrollView::ScrollView() {
  ...
  viewportArea->setOnPress([this](const InputArea::PointerData& data) {
    if (data.button != BTN_LEFT || !m_dragScrollingEnabled || !scrollable()) {
      return;
    }
    if (data.pressed) {
      stopScrollAnimation();
  ...
  viewportArea->setOnMotion([this](const InputArea::PointerData& data) {
    if (!m_dragScrollingEnabled || m_viewportArea == nullptr || !m_viewportArea->pressed() || !scrollable()) {
      return;
    }
  ...
  viewportArea->setOnAxis([this](const InputArea::PointerData& data) {
    ...
    if (m_wheelHandler && m_wheelHandler(delta)) {
      return;
    }
    if (!scrollable()) {
      return;
    ...
    const bool atBottom = delta > 0.0f && base >= m_maxScrollOffset - 0.5f;
    const bool atTop = delta < 0.0f && base <= 0.5f;
    if ((atBottom || atTop) && m_onWheelBoundary) {
      m_onWheelBoundary(delta);
      return;
    }
...
void ScrollView::scrollToOffset(float offset, bool animated, float durationMs, std::function<void()> onComplete) {
  if (animated) {
    animateScrollTo(offset, durationMs, std::move(onComplete));
    return;
  }
...
void ScrollView::animateScrollTo(float target, float durationMs, std::function<void()> onComplete) {
  ...
  m_scrollAnimId = animationManager()->animate(
      ...
      [this](float value) { applyScrollOffsetValue(value); },
      [this, onComplete = std::move(onComplete)]() mutable {
        m_scrollAnimId = 0;
        if (onComplete) {
          onComplete();
        }
...
void ScrollView::setDragScrollingEnabled(bool enabled) noexcept {
  m_dragScrollingEnabled = enabled;
  if (!enabled) {
    m_dragging = false;
    m_dragVelocity = 0.0f;
  }
...
void ScrollView::setWheelHandler(std::function<bool(float)> callback) { m_wheelHandler = std::move(callback); }
...
void ScrollView::setOnWheelBoundary(std::function<void(float)> callback) { m_onWheelBoundary = std::move(callback); }
