...
public:
  // Centralized out-of-line allocation prevents incremental links from
  // selecting a stale template instantiation with an older class size.
  [[nodiscard]] static std::unique_ptr<VirtualGridView> create();
  ...
  // Queues a non-animated scroll. This remains valid before the first layout,
  // because only the index has to be retained until cell metrics are known.
  void scrollToIndex(std::size_t index);
  // Starts an animated scroll using the current layout snapshot. Returns
  // false when layout metrics are not available yet; no callback is retained
  // inside VirtualGridView, keeping its allocation layout stable.
  bool animateToIndex(std::size_t index, std::function<void()> onComplete = {});
  // Repositions an identical physical copy without changing its viewport-relative location.
  void rebaseIndex(std::size_t fromIndex, std::size_t toIndex);
  void setSelectedIndex(std::optional<std::size_t> index, bool scrollIntoView = true);
...
private:
  ...
  [[nodiscard]] std::optional<float> scrollTargetForIndex(std::size_t index) const noexcept;
