...
public:
  ...
  // Measure the natural size of this flex tree while ignoring a size that was
  // previously assigned by its viewport owner. This is intentionally
  // different from measure(): panel content needs to discover its intrinsic
  // height after it has already been laid out inside a shorter ScrollView.
  [[nodiscard]] LayoutSize measureIntrinsic(Renderer& renderer, const LayoutConstraints& constraints);
