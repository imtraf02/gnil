...
void Input::setFlatSurfaceFrame(bool enabled) {
  if (m_flatSurfaceFrame == enabled) {
    return;
  }
  m_flatSurfaceFrame = enabled;
  applyVisualState();
}
...
void Input::applyVisualState() {
  ...
  if (m_frameVisible) {
    ...
    const Color fill = m_flatSurfaceFrame
        ? resolved(ColorRole::SurfaceVariant, m_surfaceOpacity)
        : (focused ? resolved(ColorRole::Surface, m_surfaceOpacity)
                   : resolved(ColorRole::SurfaceVariant, m_surfaceOpacity));
    const Color border = m_flatSurfaceFrame
        ? clearColor()
    ...
    m_background->setStyle(
        RoundedRectStyle{
            ...
            .softness = 1.0f,
            .borderWidth = m_flatSurfaceFrame ? 0.0f : (focused ? Style::focusRingWidth : Style::borderWidth),
        }
    );
