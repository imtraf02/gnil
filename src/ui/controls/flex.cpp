...
LayoutSize Flex::runLayout(
    Renderer& renderer, const LayoutConstraints& constraints, bool arrangeChildren, bool ignoreExplicitSize
) {
  const bool horizontal = m_direction == FlexDirection::Horizontal;
  const bool explicitWidth = !ignoreExplicitSize && width() > 0.0f && (arrangingByLayout() || m_explicitWidth);
  const bool explicitHeight = !ignoreExplicitSize && height() > 0.0f && (arrangingByLayout() || m_explicitHeight);
...
LayoutSize Flex::measureIntrinsic(Renderer& renderer, const LayoutConstraints& constraints) {
  return runLayout(renderer, constraints, false, true);
}
