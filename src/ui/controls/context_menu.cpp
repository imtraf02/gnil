...
void ContextMenuControl::setFrameJoin(std::string edge, bool joinsStartFrame, bool joinsEndFrame) {
  m_frameJoinEdge = std::move(edge);
  m_joinsStartFrame = joinsStartFrame;
  m_joinsEndFrame = joinsEndFrame;
  m_needsRebuild = true;
  markLayoutDirty();
}

void ContextMenuControl::setExternalChrome(bool external) {
  if (m_externalChrome == external) {
    return;
  }
  m_externalChrome = external;
  m_needsRebuild = true;
  markLayoutDirty();
}
    ...
    addChild(
        ui::box({
            .configure = [this](Box& bg) {
              ...
              if (!m_frameJoinEdge.empty()) {
                const float radius = Style::scaledRadiusLg(m_contentScale);
                bg.clearBorder();
                bg.setFill(colorSpecFromRole(ColorRole::Surface));
                bg.setCornerShapes(attached_panel::cornerShapes(
                    m_frameJoinEdge, m_joinsStartFrame, m_joinsEndFrame
                ));
                bg.setLogicalInset(attached_panel::logicalInset(m_frameJoinEdge, radius));
