...
public:
  ...
  GlyphGridAdapter(float chromeScale) : m_chromeScale(chromeScale) {
    ...
    seen.reserve(symbols.size() + aliases.size());
    m_master.reserve(symbols.size() + aliases.size());

    for (const auto& [name, target] : aliases) {
      if (seen.insert(name).second) {
        if (const auto it = symbols.find(std::string(target)); it != symbols.end()) {
          m_master.push_back({name, it->second});
        }
      }
    }
    for (const auto& [name, codepoint] : symbols) {
      if (seen.insert(name).second) {
