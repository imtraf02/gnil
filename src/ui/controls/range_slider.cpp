...
RangeSlider::RangeSlider() {
  ...
  auto rightTrack = std::make_unique<RectNode>();
  m_rightTrack = static_cast<RectNode*>(addChild(std::move(rightTrack)));
...
void RangeSlider::updateGeometry() {
  ...
  const bool lowActive = m_activeThumb == ActiveThumb::Low && m_inputArea != nullptr
      && (m_inputArea->pressed() || m_inputArea->focused());
  const bool highActive = m_activeThumb == ActiveThumb::High && m_inputArea != nullptr
      && (m_inputArea->pressed() || m_inputArea->focused());
  const float lowHandleWidth = lowActive ? std::max(Style::sliderHandlePressedWidth, handleRestWidth * 0.5f)
                                         : handleRestWidth;
  const float highHandleWidth = highActive ? std::max(Style::sliderHandlePressedWidth, handleRestWidth * 0.5f)
                                           : handleRestWidth;
  ...
  m_track->setFrameSize(std::max(0.0f, lowX - gap - trackX), m_trackHeight);
  m_track->setVisible(m_track->width() > 0.0f);

  m_fill->setPosition(lowX + gap, trackY);
  m_fill->setFrameSize(std::max(0.0f, highX - gap - (lowX + gap)), m_trackHeight);
  m_fill->setVisible(m_fill->width() > 0.0f);

  const float rightTrackX = highX + gap;
  m_rightTrack->setPosition(rightTrackX, trackY);
  m_rightTrack->setFrameSize(std::max(0.0f, trackX + trackW - rightTrackX), m_trackHeight);
  m_rightTrack->setVisible(m_rightTrack->width() > 0.0f);

  m_lowThumb->setPosition(util::clampOrdered(lowX - lowHandleWidth * 0.5f, 0.0f, widthPx - lowHandleWidth), thumbY);
  m_lowThumb->setFrameSize(lowHandleWidth, m_thumbSizePx);
  m_highThumb->setPosition(
      util::clampOrdered(highX - highHandleWidth * 0.5f, 0.0f, widthPx - highHandleWidth), thumbY
  );
  m_highThumb->setFrameSize(highHandleWidth, m_thumbSizePx);
...
void RangeSlider::applyVisualState() {
  ...
  m_lowThumb->setVisible(true);
  m_highThumb->setVisible(true);
