...
void Glyph::setFill(float fill) { m_glyphNode->setFill(fill); }

void Glyph::setFilled(bool filled, bool animate) {
  const float target = filled ? 1.0f : 0.0f;
  if (!animate || animationManager() == nullptr) {
    setFill(target);
    return;
  }
  const float from = m_glyphNode->fill();
  if (std::abs(from - target) < 0.001f) {
    return;
  }
  animationManager()->cancelForOwner(m_glyphNode);
  animationManager()->animate(
      from, target, 180.0f, Easing::EaseOutCubic, [this](float value) { setFill(value); }, {}, m_glyphNode
  );
}

float Glyph::fill() const noexcept { return m_glyphNode != nullptr ? m_glyphNode->fill() : 0.0f; }
...
LayoutSize Glyph::measureWithConstraints(Renderer& renderer, const LayoutConstraints& constraints) {
  ...
  // Material Symbols are designed on a square viewport. Keep layout stable by
