...
namespace scripting {
  ...
  void PluginRegistry::scan() {
    ...
    if (roots.empty()) {
      // Implicit local source: the user data dir (honoring GNIL_DATA_HOME). When
  ...
  void PluginRegistry::scanDir(const std::filesystem::path& dir) {
    ...
    for (const auto& sub : std::filesystem::directory_iterator(dir, ec)) {
      ...
      if (!noctalia::version::atLeast(noctalia::build_info::version(), manifest->minGnil)) {
        kLog.warn(
            "ignoring plugin '{}' at {}: requires Gnil >= {} (running {})", manifest->id, sub.path().string(),
            manifest->minGnil, noctalia::build_info::version()
        );
        continue;
      }
