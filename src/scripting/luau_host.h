...
public:
  ...
  // A small, host-owned process API for plugins that need an exact lifecycle
  // (e.g. media backends). Handles are local to this Luau host and are cleaned
  // up when the host is destroyed.
  [[nodiscard]] std::optional<int> startManagedProcess(std::vector<std::string> args);
  [[nodiscard]] bool isManagedProcessRunning(int handle);
  bool stopManagedProcess(int handle);
  [[nodiscard]] bool sendUnixSocket(std::string_view path, std::string_view payload) const;
...
private:
  ...
  void stopAllManagedProcesses() noexcept;
  ...
  std::unordered_map<int, int> m_managedProcesses;
  int m_nextManagedProcessHandle = 1;
