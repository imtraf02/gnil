...
namespace {
  Logger kLog{"luau"};
  constexpr const char* kHostKey = "__gnil_host";
  ...
  int luau_startProcess(lua_State* L) {
    ...
    const auto handle = host->startManagedProcess(std::move(args));
    if (!handle.has_value()) {
      lua_pushnil(L);
    } else {
  ...
  int luau_isProcessRunning(lua_State* L) {
    const int handle = static_cast<int>(luaL_checkinteger(L, 1));
    auto* host = hostForState(L);
    lua_pushboolean(L, host != nullptr && host->isManagedProcessRunning(handle) ? 1 : 0);
    return 1;
  }

  int luau_stopProcess(lua_State* L) {
    const int handle = static_cast<int>(luaL_checkinteger(L, 1));
    auto* host = hostForState(L);
    lua_pushboolean(L, host != nullptr && host->stopManagedProcess(handle) ? 1 : 0);
    return 1;
  }

  int luau_sendUnixSocket(lua_State* L) {
    size_t pathLen = 0;
    ...
    auto* host = hostForState(L);
    lua_pushboolean(
        L, host != nullptr && host->sendUnixSocket(std::string_view(path, pathLen), std::string_view(payload, payloadLen))
               ? 1
               : 0
    );
    return 1;
  ...
  const luaL_Reg kGnilBaseLib[] = {
      {"log", luau_log},
      {"runAsync", luau_runAsync},
      {"runStream", luau_runStream},
      {"runInTerminal", luau_runInTerminal},
      {"startProcess", luau_startProcess},
      {"isProcessRunning", luau_isProcessRunning},
      {"stopProcess", luau_stopProcess},
      {"sendUnixSocket", luau_sendUnixSocket},
  ...
  void registerGnilLib(lua_State* L) {
    luaL_register(L, "gnil", kGnilBaseLib);
    // gnil.state = { set, get, watch }
    lua_createtable(L, 0, 0);
    luaL_register(L, nullptr, kGnilStateLib);
    lua_setfield(L, -2, "state");
    // gnil.json = { decode, encode }
    lua_createtable(L, 0, 0);
    luaL_register(L, nullptr, kGnilJsonLib);
    lua_setfield(L, -2, "json");
    // gnil.string = { trim, urlEncode, urlDecode }
    lua_createtable(L, 0, 0);
    luaL_register(L, nullptr, kGnilStringLib);
    lua_setfield(L, -2, "string");
    // Compatibility only for existing third-party plugins. First-party Gnil
    // plugins use the gnil namespace exclusively.
    lua_pushvalue(L, -1);
    lua_setglobal(L, "noctalia");
    lua_pop(L, 1);
  }
} // namespace
...
std::optional<int> LuauHost::startManagedProcess(std::vector<std::string> args) {
  if (args.empty() || args.front().empty() || m_managedProcesses.size() >= 16) {
    return std::nullopt;
  }
  ...
  const int handle = m_nextManagedProcessHandle++;
  if (m_nextManagedProcessHandle <= 0) {
    m_nextManagedProcessHandle = 1;
  }
  m_managedProcesses.insert_or_assign(handle, *pid);
  return handle;
}

bool LuauHost::isManagedProcessRunning(int handle) {
  const auto it = m_managedProcesses.find(handle);
  if (it == m_managedProcesses.end()) {
    return false;
  }
  if (::kill(static_cast<pid_t>(it->second), 0) == 0 || errno == EPERM) {
    return true;
  }
  if (errno == ESRCH) {
...
bool LuauHost::stopManagedProcess(int handle) {
  const auto it = m_managedProcesses.find(handle);
  if (it == m_managedProcesses.end()) {
    return false;
  }
  process::terminateTracked(it->second);
  m_managedProcesses.erase(it);
  return true;
}

void LuauHost::stopAllManagedProcesses() noexcept {
  for (const auto& [_, pid] : m_managedProcesses) {
    process::terminateTracked(pid);
  }
  m_managedProcesses.clear();
}

bool LuauHost::sendUnixSocket(std::string_view path, std::string_view payload) const {
  if (path.empty() || path.size() >= sizeof(sockaddr_un::sun_path) || payload.empty() || payload.size() > 64 * 1024) {
    return false;
  }
