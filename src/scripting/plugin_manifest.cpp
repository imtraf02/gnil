...
namespace scripting {
  ...
  std::optional<PluginManifest> parsePluginManifest(const std::filesystem::path& manifestPath, std::string* error) {
    ...
    manifest.minGnil = tableString(root, "min_gnil");
    // Accept upstream manifests while first-party plugins use the Gnil name.
    if (manifest.minGnil.empty()) {
      manifest.minGnil = tableString(root, "min_noctalia");
    }
    if (manifest.minGnil.empty()) {
      return fail("missing mandatory key 'min_gnil'");
    }
