...
namespace scripting {
  ...
  namespace {
    ...
    void fillCompat(CatalogEntry& e) {
      e.compatible =
          e.minGnil.empty() || noctalia::version::atLeast(noctalia::build_info::version(), e.minGnil);
    }
  ...
  std::vector<CatalogEntry> parseCatalogToml(const std::string& body) {
    ...
    for (const auto& node : *plugins) {
      ...
      CatalogEntry e{
          ...
          .minGnil = tableString(*tbl, "min_gnil"),
      ...
      // Existing third-party catalogs still use the upstream spelling.
      if (e.minGnil.empty()) {
        e.minGnil = tableString(*tbl, "min_noctalia");
      }
      fillCompat(e);
