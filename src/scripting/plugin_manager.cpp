...
namespace scripting {
  ...
  namespace {
    ...
    PluginSourceConfig builtinPluginSource() {
      return {
          .kind = PluginSourceKind::Path,
          .name = "builtin",
          .location = paths::assetPath("plugins").string(),
          .enabled = true,
      };
    }
    ...
    MaterializeResult materializeFailure(
      ...
      result.requiredGnil = std::move(requiredGnil);
      return result;
    }
    ...
    MaterializeResult materializeGitPlugin(
      ...
      if (requireCompatible && !noctalia::version::atLeast(noctalia::build_info::version(), manifest->minGnil)) {
        std::string error = "plugin '"
            ...
            + ")";
        const std::string required = manifest->minGnil;
        cleanupTmp();
    ...
    bool materializedPluginMatchesCatalog(
      ...
      return manifest->version == entry.version && manifest->minGnil == entry.minGnil;
    }
  } // namespace
  ...
  void applyPluginSourcesToRegistry(PluginRegistry& registry, const PluginsConfig& plugins) {
    ...
    if (auto root = sourceRootFor(builtinPluginSource()); !root.empty()) {
  ...
  bool PluginManager::materializeEnabledFromRepo(
    ...
    for (const auto& id : enabled) {
      ...
      if (catalogEntry != nullptr && !catalogEntry->compatible) {
        if (!hasMaterialized) {
          kLog.warn(
              "plugin source '{}': cannot export enabled plugin '{}'; it requires Gnil >= {} (running {})",
              source.name, id, catalogEntry->minGnil, noctalia::build_info::version()
          );
        }
        continue;
      }
      ...
      if (materializedPlugin) {
        ...
        kLog.warn(
            "plugin source '{}': cannot export enabled plugin '{}'; it requires Gnil >= {} (running {})",
            source.name, id, materializedPlugin.requiredGnil, noctalia::build_info::version()
        );
  ...
  EnableResult PluginManager::enable(std::string_view pluginId) {
    ...
    std::thread([this, id, subdir = *subdir, sources = std::move(sources)]() mutable {
      ...
      // Built-ins deliberately have the lowest precedence. They are still
      // resolved here so enabling one never needs a source clone or network.
      if (!offering.has_value()) {
        const auto builtin = builtinPluginSource();
        const auto catalog = discoverCatalog(builtin);
        if (std::ranges::any_of(catalog.entries, [&](const CatalogEntry& e) { return e.id == id; })) {
          offering = builtin;
        }
      }
      ...
      if (offering.has_value() && offering->kind == PluginSourceKind::Git) {
        ...
        requiredGnil = std::move(materialized.requiredGnil);
        ...
        if (manifest.has_value() && manifest->id != id) {
          error = "manifest id '" + manifest->id + "' does not match requested id";
            ...
            manifest.has_value() && !noctalia::version::atLeast(noctalia::build_info::version(), manifest->minGnil)
        ) {
          incompatible = true;
          requiredGnil = manifest->minGnil;
        } else {
      ...
      DeferredCall::callLater([this, id, ok, incompatible, timedOut, elapsedMs,
                               requiredGnil = std::move(requiredGnil), error = std::move(error)]() mutable {
        m_enabling.erase(id);
        if (ok) {
          ...
          kLog.warn(
              "cannot enable '{}': requires Gnil >= {} (running {})", id, requiredGnil,
              noctalia::build_info::version()
          );
          notify::error(
              "Gnil", i18n::tr("plugins.enable-failed.title"),
              i18n::tr("plugins.enable-failed.body-incompatible", "plugin", id, "version", requiredGnil)
          );
        } else if (timedOut) {
  ...
  std::vector<PluginStatus> PluginManager::list(const PluginsConfig& plugins) const {
    ...
    const auto builtin = builtinPluginSource();
    collect("builtin", discoverCatalog(builtin), builtin);
    return out;
  }
  ...
  void PluginManager::update(std::string sourceName) {
    ...
    std::thread([this, source = *source, repoRoot, sourceName = std::move(sourceName),
      ...
      const auto rememberWithheld = [&](std::string id, std::string minGnil) {
        if (withheldIds.insert(id).second) {
          withheldUpdates.emplace_back(std::move(id), std::move(minGnil));
        }
      };
      ...
      for (const auto& id : enabled) {
        ...
        const auto* catalogEntry = findCatalogEntry(catalog, id);
        if (catalogEntry != nullptr && !catalogEntry->compatible) {
          rememberWithheld(id, catalogEntry->minGnil);
          continue;
        }
        ...
        if (const auto m = materializeGitPlugin(source, repoRoot, newRev, id, true); !m) {
          if (m.incompatible) {
            rememberWithheld(id, m.requiredGnil);
            continue;
          }
      ...
      DeferredCall::callLater([this, sourceName, ok = applied, err = std::move(applyError), newRev,
        ...
        for (const auto& [id, min] : withheldUpdates) {
          kLog.warn(
              "update '{}': kept previous '{}' export; new version requires Gnil >= {} (running {})", sourceName,
