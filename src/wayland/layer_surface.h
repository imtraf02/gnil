...
// The layer-shell v5 exclusive-edge request uses the same bit values as the
// anchor enum. Zero keeps the protocol's automatic edge inference for older
// or unambiguous surfaces.
namespace LayerShellExclusiveEdge {
  inline constexpr std::uint32_t Automatic = 0;
  inline constexpr std::uint32_t Top = LayerShellAnchor::Top;
  inline constexpr std::uint32_t Bottom = LayerShellAnchor::Bottom;
  inline constexpr std::uint32_t Left = LayerShellAnchor::Left;
  inline constexpr std::uint32_t Right = LayerShellAnchor::Right;
} // namespace LayerShellExclusiveEdge

struct LayerSurfaceConfig {
  std::string nameSpace = "gnil";
  LayerShellLayer layer = LayerShellLayer::Top;
  std::uint32_t anchor = 0;
  std::uint32_t width = 0;
  std::uint32_t height = 0;
  std::int32_t exclusiveZone = 0;
  std::uint32_t exclusiveEdge = LayerShellExclusiveEdge::Automatic;
