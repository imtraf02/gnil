...
bool LayerSurface::initialize(wl_output* output) {
  ...
  if (m_config.exclusiveEdge != LayerShellExclusiveEdge::Automatic
      && wl_proxy_get_version(reinterpret_cast<wl_proxy*>(m_layerSurface))
          >= ZWLR_LAYER_SURFACE_V1_SET_EXCLUSIVE_EDGE_SINCE_VERSION) {
    zwlr_layer_surface_v1_set_exclusive_edge(m_layerSurface, m_config.exclusiveEdge);
  }
