...
void RenderContext::initialize(GlSharedContext& shared) {
  ...
  m_glyphRenderer.initialize(
      paths::assetPath("fonts/material-symbols-rounded.ttf").string(), m_backend.get(), &m_backend->textureManager()
  );
...
void RenderContext::renderNode(
  ...
  case NodeType::ChromeBlob: {
    const auto* chrome = static_cast<const ChromeBlobNode*>(node);
    auto style = chrome->style();
    style.fill.a *= effectiveOpacity;
    style.shadow.a *= effectiveOpacity;
    const float pixelScaleX = sw > 0.0f ? bw / sw : 1.0f;
    const float pixelScaleY = sh > 0.0f ? bh / sh : 1.0f;
    m_backend->drawChromeBlob(
        sw, sh, pixelScaleX, pixelScaleY, node->width(), node->height(), style, worldTransform
    );
    break;
  }
  case NodeType::Text: {
    ...
    if (icon->codepoint() != 0) {
      if (icon->hasShadow()) {
        ...
        m_glyphRenderer.drawGlyph(
            sw, sh, 0.0f, 0.0f, icon->codepoint(), icon->fontSize(), icon->fill(), shadowColor, shadowTransform
        );
      }
      auto color = icon->color();
      color.a *= effectiveOpacity;
      m_glyphRenderer.drawGlyph(
          sw, sh, 0.0f, 0.0f, icon->codepoint(), icon->fontSize(), icon->fill(), color, worldTransform
      );
    }
    break;
  }
