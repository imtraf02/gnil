...
namespace {
  ...
  constexpr std::uint8_t kFillSteps = 8;
  ...
  inline std::uint8_t quantizeFill(float value) {
    return static_cast<std::uint8_t>(std::lround(std::clamp(value, 0.0f, 1.0f) * kFillSteps));
  }

  cairo_font_options_t* materialFontOptions(const cairo_font_options_t* base, std::uint8_t fillQ, float fontSize) {
    cairo_font_options_t* options = cairo_font_options_copy(base);
    const float fill = static_cast<float>(fillQ) / static_cast<float>(kFillSteps);
    const float opticalSize = std::clamp(fontSize, 20.0f, 48.0f);
    const std::string variations = std::format("FILL={},wght=500,GRAD=0,opsz={}", fill, opticalSize);
    cairo_font_options_set_variations(options, variations.c_str());
    return options;
  }
  ...
  // Hinting is disabled so variation-axis interpolation does not jump between
  // integer stroke widths while an icon morphs from outline to filled.
  cairo_scaled_font_t*
...
bool CairoGlyphRenderer::CacheKey::operator==(const CacheKey& other) const noexcept {
  return codepoint == other.codepoint && sizeQ == other.sizeQ && scaleQ == other.scaleQ && fillQ == other.fillQ;
}
...
std::size_t CairoGlyphRenderer::CacheKeyHash::operator()(const CacheKey& k) const noexcept {
  ...
  hashCombine(seed, std::hash<std::uint8_t>{}(k.fillQ));
  return seed;
}
...
CairoGlyphRenderer::TextMetrics CairoGlyphRenderer::measureGlyph(char32_t codepoint, float fontSize) {
  ...
  cairo_font_options_t* options = materialFontOptions(m_fontOptions, 0, fontSize);
  cairo_scaled_font_t* scaledFont = create_scaled_font(m_cairoFace, options, rasterSize);
...
CairoGlyphRenderer::CacheEntry* CairoGlyphRenderer::lookupOrRasterize(char32_t codepoint, float fontSize, float fill) {
  CacheKey key;
  key.codepoint = codepoint;
  key.sizeQ = quantizeSize(fontSize);
  key.scaleQ = quantizeScale(m_contentScale);
  key.fillQ = quantizeFill(fill);
  ...
  cairo_font_options_t* options = materialFontOptions(m_fontOptions, key.fillQ, fontSize);
  cairo_scaled_font_t* scaledFont = create_scaled_font(m_cairoFace, options, rasterSize);
  cairo_font_options_destroy(options);
  if (scaledFont == nullptr || cairo_scaled_font_status(scaledFont) != CAIRO_STATUS_SUCCESS) {
    if (scaledFont != nullptr) {
...
void CairoGlyphRenderer::drawGlyph(
    float surfaceWidth, float surfaceHeight, float x, float baselineY, char32_t codepoint, float fontSize, float fill,
  ...
  CacheEntry* entry = lookupOrRasterize(codepoint, fontSize, fill);
