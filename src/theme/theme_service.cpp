...
namespace noctalia::theme {
  ...
  void ThemeService::setLiveWallpaperPaletteProvider(LiveWallpaperPaletteProvider provider) {
    m_liveWallpaperPaletteProvider = std::move(provider);
  }
  ...
  std::optional<GeneratedPalette> ThemeService::resolveLiveWallpaperGenerated(
      const ThemeConfig& cfg, const LiveWallpaperPaletteSource& source
  ) {
    if (source.identity.empty() || source.framePaths.empty()) {
      return std::nullopt;
    }
    if (m_liveWallpaperCacheGenerated.has_value()
        && m_liveWallpaperCacheIdentity == source.identity
        && m_liveWallpaperCacheScheme == cfg.wallpaperScheme) {
      return m_liveWallpaperCacheGenerated;
    }
    ...
    auto image = buildLiveWallpaperMosaic(source.framePaths, *scheme);
    if (!image.has_value()) {
      kLog.warn("failed to build live wallpaper palette sample: {}", image.error());
      return std::nullopt;
    }
    auto generated = generate(image->rgb, *scheme);
    if (!generated.has_value()) {
      kLog.warn("failed to generate live wallpaper palette: {}", generated.error());
      return std::nullopt;
    }
    m_liveWallpaperCacheGenerated = *generated;
    m_liveWallpaperCacheIdentity = source.identity;
    m_liveWallpaperCacheScheme = cfg.wallpaperScheme;
    return *generated;
  }
  ...
  void ThemeService::resolveAndSet(bool animate) {
    ...
    if (cfg.source == PaletteSource::Custom && !cfg.customPalette.empty()) {
      ...
      if (m_liveWallpaperPaletteProvider) {
        if (auto liveSource = m_liveWallpaperPaletteProvider(cfg.liveWallpaperOutput)) {
          generated = resolveLiveWallpaperGenerated(cfg, *liveSource);
        }
      }
      // Frame extraction is asynchronous. Retain the static wallpaper palette
      // as an immediate startup fallback until the stable video sample arrives.
      if (!generated.has_value()) {
        generated = resolveWallpaperGenerated(cfg, m_config.getPaletteWallpaperPath());
      }
      if (generated.has_value()) {
