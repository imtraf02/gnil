...
namespace {
  ...
  int runTopLevelFlag(const char* flag) {
    ...
    if (std::strcmp(flag, "--help") == 0 || std::strcmp(flag, "-h") == 0) {
      std::println(
          ...
          "                   Run 'gnil msg --help' for available commands\n"
  ...
  int runShell() {
    ...
    if (niriSocket == nullptr || niriSocket[0] == '\0') {
      std::println(stderr, "error: GNIL only runs inside a Niri session (NIRI_SOCKET is not set)");
    ...
    if (!instanceLock.tryAcquire()) {
      std::println(stderr, "error: gnil is already running");
      completeDaemonStartup(1);
      _exit(1);
    }
    try {
...
int main(int argc, char* argv[]) {
  ...
  if (shouldDaemonize && !isDaemonChild) {
    ...
    if (result == SpawnResult::Parent) {
      ...
      if (daemonResult == 0) {
        std::println("gnil started [pid: {}]", pid);
      }
      return daemonResult;
    }
  }
