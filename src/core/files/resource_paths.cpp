  ...
  namespace {
    ...
    bool isAssetRoot(const std::filesystem::path& root) {
      ...
      return std::filesystem::exists(root / "emoji.json", ec)
          && std::filesystem::exists(root / "fonts" / "material-symbols-rounded.ttf", ec)
          && std::filesystem::exists(root / "fonts" / "material-symbols-rounded.codepoints", ec)
    ...
    std::vector<std::filesystem::path> assetCandidates() {
      ...
      if (const char* env = std::getenv("GNIL_ASSETS_DIR"); env != nullptr && env[0] != '\0') {
        const std::filesystem::path overridePath(env);
        if (isAssetRoot(overridePath)) {
          candidates.push_back(overridePath);
          return candidates;
        }
        kLog.warn("GNIL_ASSETS_DIR is not a valid asset bundle: {}", overridePath.string());
      }
      ...
      if (auto exe = executablePath()) {
        ...
        appendUnique(candidates, exeDir.parent_path() / "share" / "gnil" / "assets");
      }
