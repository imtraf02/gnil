...
namespace gnil::config {
  ...
  schema::Diagnostics validateConfigSources(std::string_view configDir, std::string_view settingsTomlPath) {
    const std::filesystem::path settingsPath = settingsTomlPath.empty()
        ? std::filesystem::path(std::string(configDir)) / "settings.toml"
        : std::filesystem::path(std::string(settingsTomlPath));
    if (std::filesystem::exists(settingsPath)) {
      return validateConfigFile(settingsPath.string());
    }
    schema::Diagnostics diag;
    diag.fatal("settings.toml", "settings.toml does not exist", "config.settings.missing");
    return diag;
  }
  ...
  schema::Diagnostics validateConfigFile(std::string_view path) {
    ...
    if (parsed.contains("schema_version")) {
      std::string error;
      auto runtime = gnil::settings_document::toRuntimeOverrides(parsed, &error);
      if (!runtime.has_value()) {
        diag.fatal("schema_version", error, "config.schema-version");
        return diag;
      }
      runtime->erase(kConfigVersionKey);
      appendMergedConfigDiagnostics(*runtime, diag);
      return diag;
    }
