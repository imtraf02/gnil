    ...
    return Field<Struct>{
        ...
        [member, key](const toml::table& tbl, Struct& out, std::string_view, Diagnostics&) {
          if (auto* arr = tbl[key].as_array()) {
            // Build directly in the destination.  Moving a temporary vector
            // here exercises libstdc++'s allocator-propagating move path,
            // which is unsafe in the current debug toolchain even for an
            // empty array.  Replacing the list in place is equivalent and
            // keeps fresh profiles deterministic.
            auto& values = out.*member;
            values.clear();
