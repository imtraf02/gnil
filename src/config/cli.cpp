...
namespace gnil::config {
  namespace {
    ...
    constexpr const char* kHelpText =
        "Usage: gnil config <command> [options]\n"
        "\n"
        "Commands:\n"
        "  validate [path]\n"
        "      Check config validity: TOML syntax, unknown/misspelled settings, and bad\n"
        "      values. Defaults to the active config-dir settings.toml. A directory\n"
        "      validates its settings.toml; a file validates only that file. Exit 1 on error.\n"
        "\n"
        "  export [merged|full]\n"
        ...
        "      Reconstruct config-home/gnil and state-home/gnil from a support report.\n"
        "\n"
        "  replay-report <report.toml> --target <dir> --flattened [--force]\n"
        "      Reconstruct a single config-home/gnil/settings.toml from the report's merged config.\n";

    constexpr const char* kValidateHelpText =
        "Usage: gnil config validate [path]\n"
        "\n"
        "With no path, validates the merged configuration the way the shell loads it:\n"
        "  - the active config-dir settings.toml.\n"
        "\n"
        "With a directory path, validates that directory's settings.toml.\n"
    ...
    constexpr const char* kReplayHelpText =
        "Usage: gnil config replay-report <report.toml> --target <dir> [--flattened] [--force]\n"
        "\n"
        "Options:\n"
        "  --target <dir>  Directory where replay files are written\n"
        "  --flattened     Write only merged_config.content as settings.toml\n"
        "  --force         Remove an existing target directory before writing\n";

    constexpr const char* kExportHelpText = "Usage: gnil config export [merged|full]\n"
                                            "\n"
                                            "Prints TOML from the same config-dir settings.toml used by the shell.\n"
                                            "\n"
                                            "Modes:\n"
    ...
    constexpr const char* kSettingsCountHelpText =
        "Usage: gnil config settings-count\n"
        "\n"
    ...
    int runSettingsCount(int argc, char* argv[]) {
      for (int i = 3; i < argc; ++i) {
        ...
        std::println(stderr, "Run 'gnil config settings-count --help' for usage.");
        return 1;
      }
    ...
    int replayReport(const ReplayOptions& options, const char* argv0) {
      ...
      const std::filesystem::path configDir = configHome / "gnil";
      const std::filesystem::path stateDir = stateHome / "gnil";
      ...
      if (options.flattened) {
        ...
        if (auto written = writeTextFile(configDir / "settings.toml", *merged); !written) {
      ...
      std::println(
          "  GNIL_CONFIG_HOME={} GNIL_STATE_HOME={} {}", StringUtils::shellQuote(configHome.string()),
    ...
    int runValidate(int argc, char* argv[]) {
      ...
      for (int i = 3; i < argc; ++i) {
        ...
        std::println(stderr, "error: unexpected argument: {}", argv[i]);
        std::println(stderr, "Run 'gnil config validate --help' for usage.");
        return 1;
      }
      ...
      if (pathArg.empty()) {
        ...
        const std::string settingsPath = configDir.empty() ? std::string{} : configDir + "/settings.toml";
    ...
    int runExport(int argc, char* argv[]) {
      ...
      for (int i = 3; i < argc; ++i) {
        ...
        std::println(stderr, "error: unexpected argument: {}", argv[i]);
        std::println(stderr, "Run 'gnil config export --help' for usage.");
        return 1;
      }
  ...
  int runCli(int argc, char* argv[]) {
    ...
    if (std::strcmp(argv[2], "replay-report") == 0) {
      ...
      if (!parsed) {
        ...
        std::println(stderr, "Run 'gnil config replay-report --help' for usage.");
        return 1;
      }
    ...
    std::println(stderr, "Run 'gnil config --help' for usage.");
    return 1;
  }
