...
int main() {
  ...
  // Intrinsic measurement must ignore a viewport height assigned during an
  // earlier layout. Bar panels depend on this when live content grows or
  // shrinks after their ScrollView has already been constrained.
  {
    Flex column;
    column.setDirection(FlexDirection::Vertical);
    column.setGap(6.0f);
    auto firstBox = std::make_unique<Box>();
    firstBox->setSize(100.0f, 40.0f);
    column.addChild(std::move(firstBox));
    column.setSize(100.0f, 360.0f);

    auto constraints = LayoutConstraints::unconstrained();
    constraints.setExactWidth(100.0f);
    const LayoutSize first = column.measureIntrinsic(renderer, constraints);
    ok = expect(std::abs(first.height - 40.0f) < 0.01f, "intrinsic height ignores pinned viewport") && ok;

    auto secondBox = std::make_unique<Box>();
    secondBox->setSize(100.0f, 24.0f);
    column.addChild(std::move(secondBox));
    const LayoutSize grown = column.measureIntrinsic(renderer, constraints);
    ok = expect(std::abs(grown.height - 70.0f) < 0.01f, "intrinsic height follows added content") && ok;
  }

  // VirtualGridView must be safe before its first layout. WallpaperPanel and
  // LauncherPanel both clear selection during onOpen(), which previously wrote
  // beyond an under-allocated object selected from a stale weak allocator.
  {
    auto grid = VirtualGridView::create();
    grid->setSelectedIndex(std::nullopt, false);
    grid->scrollToIndex(0);
    grid->setSelectedIndex(0, false);
    grid->setSelectedIndex(std::nullopt, false);
    ok = expect(grid->selectedIndex() == std::nullopt, "virtual grid pre-layout lifecycle is safe") && ok;
  }
