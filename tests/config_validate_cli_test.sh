...
if valid_output=$("$gnil_bin" config validate tests/config_validate/generated-config 2>&1); then
  :
else
  status=$?
...
warn_output=$("$gnil_bin" config validate tests/config_validate/warn-only.toml 2>&1) \
...
syntax_output=$("$gnil_bin" config validate tests/config_validate/syntax-error.toml 2>&1) \
...
timezone_output=$("$gnil_bin" config validate tests/config_validate/invalid-timezone.toml 2>&1) \
...
XDG_CONFIG_HOME="$export_dir/config" XDG_STATE_HOME="$export_dir/state" \
  "$gnil_bin" config export full > "$export_dir/full.toml" \
  || fail "config export full failed"

if export_output=$("$gnil_bin" config validate "$export_dir/full.toml" 2>&1); then
  :
else
  status=$?
